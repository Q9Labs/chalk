...
- **New image not running** – Cause: The `latest` tag does not trigger an update. Fix: Force a new deployment or use SHA-based tags.
...
- **API Gateway 5xx** – Cause: TLS SNI mismatch. Fix: `ServerNameToVerify` must match the ACM certificate, not the ALB DNS name.
- **DB connection refused** – Cause: Missing security group. Fix: ECS has two security groups (instances and tasks) – allow both.
