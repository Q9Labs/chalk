...
func main() {
	...
	opsService := infraops.NewService(pool, queries, redisClient, cfg, slog.Default())
	...
	if transcriptionService != nil {
		...
		startWorker(func(workerCtx context.Context) {
			runPeriodicHeartbeat(workerCtx, 30*time.Second, func(loopCtx context.Context) error {
				return opsService.RecordInternalHeartbeat(loopCtx, "transcription.worker", map[string]any{"source": "main"})
			})
		})
	}
	...
	startWorker(func(workerCtx context.Context) {
		runPeriodicHeartbeat(workerCtx, 30*time.Second, func(loopCtx context.Context) error {
			return opsService.RecordInternalHeartbeat(loopCtx, "webhook.delivery.worker", map[string]any{"source": "main"})
		})
	})
	...
	startWorker(func(workerCtx context.Context) {
		runPeriodicHeartbeat(workerCtx, 30*time.Minute, func(loopCtx context.Context) error {
			return opsService.RecordInternalHeartbeat(loopCtx, "internal_retention.job", map[string]any{"source": "main"})
		})
	})

	startWorker(func(workerCtx context.Context) {
		opsService.RunHeartbeatEvaluator(workerCtx, time.Minute)
	})
	slog.Info("started ops heartbeat evaluator", "interval", "1m")

	startWorker(func(workerCtx context.Context) {
		opsService.RunNotificationWorker(workerCtx, 30*time.Second)
	})
	slog.Info("started ops notification worker", "interval", "30s")

	startWorker(func(workerCtx context.Context) {
		opsService.RunRetentionWorker(workerCtx, 12*time.Hour)
	})
	slog.Info("started ops retention worker", "interval", "12h")
...
func runPeriodicHeartbeat(ctx context.Context, interval time.Duration, emit func(context.Context) error) {
	ticker := time.NewTicker(interval)
	defer ticker.Stop()

	for {
		if err := emit(ctx); err != nil {
			slog.Warn("ops heartbeat emit failed", "error", err)
		}
		select {
		case <-ctx.Done():
			return
