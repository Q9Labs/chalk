...
func (t *postgresBootstrapTransaction) SpaceBySlug(ctx context.Context, tenantID utilities.ID, slug string) (spaces.Space, bool, error) {
	var (
		spaceID    string
		name       string
		mediaPlane string
...
select id::text, name, media_plane
from spaces
where tenant_id = $1::uuid and slug = $2`, tenantID.String(), slug).Scan(&spaceID, &name, &mediaPlane)
	if errors.Is(err, pgx.ErrNoRows) {
		return spaces.Space{}, false, nil
	}
	if err != nil {
		return spaces.Space{}, false, err
	}
	id, err := utilities.ParseID(spaceID)
	if err != nil {
		return spaces.Space{}, false, err
	}
	return spaces.Space{ID: id, TenantID: tenantID, Name: name, Slug: slug, MediaPlane: mediaPlane}, true, nil
}

func (t *postgresBootstrapTransaction) CreateSpace(ctx context.Context, input spaces.CreateSpaceInput) (spaces.Space, error) {
	repository := postgres.NewSpaceRepository(t.queries)
	return spaces.NewService(repository).CreateSpace(ctx, input)
}

func (t *postgresBootstrapTransaction) Commit(ctx context.Context) error {
	if err := t.tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit space bootstrap: %w", err)
	}
	return nil
}
