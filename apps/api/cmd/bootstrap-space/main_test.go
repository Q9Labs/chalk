...
func TestExecuteNormalizesIdentityFlags(t *testing.T) {
	...
	err := execute(context.Background(), []string{
		...
		"--space-name", "  Custom space  ",
		"--space-slug", "  custom-space  ",
	...
	if transaction.createdTenantInput.Name != "Custom tenant" || transaction.createdSpaceInput.Name != "Custom space" || transaction.createdSpaceInput.Slug != "custom-space" || transaction.createdKeyInput.Name != "custom-key" {
		t.Fatalf("normalized resources = %q / %q / %q / %q", transaction.createdTenantInput.Name, transaction.createdSpaceInput.Name, transaction.createdSpaceInput.Slug, transaction.createdKeyInput.Name)
	}
}
...
func TestExecuteRollsBackAndEmitsNothingOnProvisionFailure(t *testing.T) {
	transaction := &fakeBootstrapTransaction{
		...
		spaceFound:  true,
		space: spaces.Space{
			ID: bootstrapTestSpaceID, Name: "incompatible space", Slug: defaultSpaceSlug, MediaPlane: cloudflareSFU,
		},
	}
	database := &fakeBootstrapDatabase{transaction: transaction}
	var output bytes.Buffer
	...
	if err == nil || err.Error() != "bootstrap space slug is already used by an incompatible space" {
