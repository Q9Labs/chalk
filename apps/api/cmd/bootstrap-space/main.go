...
func main() {
	...
	if err := execute(ctx, os.Args[1:], os.Getenv, os.Stdout, openBootstrapDatabase, time.Now); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "bootstrap space: %v\n", err)
		os.Exit(1)
	}
}
...
func execute(ctx context.Context, arguments []string, environment environmentReader, output io.Writer, open databaseOpener, now func() time.Time) error {
	...
	result, err := bootstrapSpace(ctx, transaction, bootstrapInput{
		TenantName: options.TenantName, SpaceName: options.SpaceName, SpaceSlug: options.SpaceSlug,
...
func parseOptions(arguments []string) (commandOptions, error) {
	...
	flags := flag.NewFlagSet("bootstrap-space", flag.ContinueOnError)
	...
	flags.StringVar(&options.SpaceName, "space-name", defaultSpaceName, "bootstrap space name")
	flags.StringVar(&options.SpaceSlug, "space-slug", defaultSpaceSlug, "bootstrap space slug")
...
func prepareIdentityOptions(options *commandOptions) error {
	values := []struct {
		...
		{name: "--space-name", value: &options.SpaceName},
		{name: "--space-slug", value: &options.SpaceSlug},
