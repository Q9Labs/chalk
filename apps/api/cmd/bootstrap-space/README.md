# Episode broker bootstrap

`bootstrap-space` is the narrow operator boundary for creating the first
production episode-broker tenant. It connects only through
`CHALK_DATABASE_URL`, takes a transaction-scoped advisory lock, and commits the
tenant, owner membership, broker API key, and space together.
...
The default names can be changed with `--tenant-name`, `--space-name`,
`--space-slug`, and `--api-key-name`. `--api-key-ttl` accepts a Go duration and
