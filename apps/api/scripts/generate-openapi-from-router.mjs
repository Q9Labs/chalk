...
const ROUTE_RE = /^\s*([A-Za-z0-9_.]+)\.(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\("([^"]*)"/;
...
function createStubOperation(method, openapiPathValue) {
  return {
    ...
    description: "Auto-generated from router.go because this operation is not documented yet. Replace with explicit request/response schema.",
  ...
  for (const [pathValue, methods] of [...methodsByPath.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    ...
    for (const method of [...methods].sort()) {
      if (existingPathItem[method]) {
        ...
        nextPathItem[method] = isLikelyGeneratedStub(existingOperation) ? normalizeStubOperation(existingOperation, method, pathValue) : existingOperation;
