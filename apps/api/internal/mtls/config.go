...
func LoadClientConfig(certificateFile string, privateKeyFile string, serverCAFile string, serverName string) (*tls.Config, error) {
	return loadClientConfig(certificateFile, privateKeyFile, serverCAFile, serverName, false)
}

// LoadReloadingClientConfig reloads the client key pair for each new TLS
// handshake. Recorder nodes use it so certificate renewal does not interrupt
// an active render attempt or require a daemon restart.
func LoadReloadingClientConfig(certificateFile string, privateKeyFile string, serverCAFile string, serverName string) (*tls.Config, error) {
	return loadClientConfig(certificateFile, privateKeyFile, serverCAFile, serverName, true)
}

func loadClientConfig(certificateFile string, privateKeyFile string, serverCAFile string, serverName string, reload bool) (*tls.Config, error) {
	certificateFile = strings.TrimSpace(certificateFile)
	privateKeyFile = strings.TrimSpace(privateKeyFile)
	...
	config := &tls.Config{RootCAs: serverCAs, ServerName: serverName, MinVersion: tls.VersionTLS13}
	if !reload {
		config.Certificates = []tls.Certificate{certificate}
		return config, nil
	}
	config.GetClientCertificate = func(*tls.CertificateRequestInfo) (*tls.Certificate, error) {
		current, err := tls.LoadX509KeyPair(certificateFile, privateKeyFile)
		if err != nil {
			return nil, fmt.Errorf("%w: reload client certificate: %v", ErrInvalidConfig, err)
		}
		return &current, nil
	}
	return config, nil
