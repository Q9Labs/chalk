...
func TestHubCanParticipantAnnotate_AccessModes(t *testing.T) {
	mockRedis := &MockRedisClient{}
	hub := NewHub(mockRedis, nil)
	defer hub.Close()
	...
	assert.True(t, hub.CanParticipantAnnotate(roomID, sharerID))
	assert.True(t, hub.CanParticipantAnnotate(roomID, participantID))

	accessMode, ok := hub.SetScreenAnnotationAccessMode(roomID, "share-1", AnnotationAccessModeSharerOnly)
	assert.True(t, ok)
	assert.Equal(t, AnnotationAccessModeSharerOnly, accessMode)
	assert.True(t, hub.CanParticipantAnnotate(roomID, sharerID))
	assert.False(t, hub.CanParticipantAnnotate(roomID, participantID))

	accessMode, ok = hub.SetScreenAnnotationAccessMode(roomID, "share-1", AnnotationAccessModeOff)
	assert.True(t, ok)
	assert.Equal(t, AnnotationAccessModeOff, accessMode)
	assert.False(t, hub.CanParticipantAnnotate(roomID, sharerID))
	assert.False(t, hub.CanParticipantAnnotate(roomID, participantID))
	assert.True(t, hub.isHostParticipant(hostID))
}
