...
func TestDebugClientIncident_BadPayload_400(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/debug/client-incident", bytes.NewBufferString(`{"source":"example-client"}`))
...
func TestDebugClientIncident_Envelope_OK_202(t *testing.T) {
	...
	body := `{
		"incident": {
			...
			"retryable": true,
			"context": {
				"url": "https://example.edu/dashboard/class-room/69949e183ef506489ede4a08",
