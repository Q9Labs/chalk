...
func NewRouter(cfg RouterConfig) *Router {
	...
	opsService := infraops.NewService(cfg.Pool, queries, cfg.RedisClient, cfg.AppConfig, slog.Default())
...
func (r *Router) setupRoutes() {
	...
	v1 := r.engine.Group("/api/v1")
	{
		...
		opsPublic := handlers.NewOpsPublicHandler(r.opsService)
		v1.GET("/status", opsPublic.StatusSummary)
		v1.GET("/status/incidents/:incidentCode", opsPublic.Incident)
		v1.GET("/status/card.png", opsPublic.StatusCard)

		opsIngest := handlers.NewOpsIngestHandler(r.opsService)
		opsIngestMw := middleware.NewOpsIngestMiddleware(&r.appConfig.Ops)
		opsGroup := v1.Group("/ops")
		opsGroup.Use(opsIngestMw.RequireIngestToken())
		{
			opsGroup.POST("/ingest/monitor-results", opsIngest.MonitorResult)
			opsGroup.POST("/ingest/heartbeats", opsIngest.HeartbeatEvent)
		}
		...
		if r.appConfig.Admin.Enabled {
			...
			opsAdmin := handlers.NewOpsAdminHandler(r.opsService)
			...
			admin.Use(adminMw.RequireAdmin())
			{
				...
				admin.GET("/ops/overview", opsAdmin.Overview)
				admin.GET("/ops/incidents", opsAdmin.ListIncidents)
				admin.GET("/ops/incidents/:incidentCode", opsAdmin.GetIncident)
				admin.POST("/ops/incidents/declare", opsAdmin.DeclareIncident)
				admin.POST("/ops/incidents/:incidentCode/events", opsAdmin.AddEvent)
				admin.POST("/ops/incidents/:incidentCode/publish", opsAdmin.PublishIncident)
				admin.POST("/ops/incidents/:incidentCode/resolve", opsAdmin.ResolveIncident)
				admin.POST("/ops/incidents/:incidentCode/ai-drafts", opsAdmin.AIDrafts)
				admin.POST("/ops/maintenance", opsAdmin.CreateMaintenance)
				admin.POST("/ops/maintenance/:id/cancel", opsAdmin.CancelMaintenance)
			}
		}
	}
}
