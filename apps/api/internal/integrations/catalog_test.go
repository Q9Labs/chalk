...
func TestCatalogRejectsDuplicateServiceIDs(t *testing.T) {
	_, err := NewCatalog([]ServiceEntry{
		...
		{ID: "slack", Family: "Work", DisplayName: "Slack Again", Provider: ProviderComposio, ToolkitSlug: "slack", AllowedActions: []ActionPolicy{{ID: "send_message", Slug: "SLACK_SEND_MESSAGE", DisplayName: "Send message"}}},
	})
