...
func runWebhookDeliveryAttempt(context.Context) (ScenarioResult, error) {
	...
	body := []byte(`{"id":"11111111-1111-4111-8111-111111111115","event":"space.created","api_version":1,"occurred_at":"2026-07-12T19:13:00.000Z","tenant_id":"22222222-2222-4222-8222-222222222221","data":{"object":{"id":"33333333-3333-4333-8333-333333333331","name":"Design review","slug":"design-review","media_plane":"cf_rtk","created_at":"2026-07-12T19:13:00.000Z","updated_at":"2026-07-12T19:13:00.000Z"}}}`)
	digest := sha256.Sum256(body)
	recorder.Add("database", "webhook.producer.transaction_started", "begin authoritative space mutation", map[string]any{"isolation": "read_committed", "resource_type": "space"})
	recorder.Add("database", "space.created", "insert authoritative space state", map[string]any{})
	recorder.Add("database", "webhook.event.inserted", "insert immutable signed event body", map[string]any{"body_bytes": len(body), "body_sha256": fmt.Sprintf("%x", digest[:8]), "event_name": "space.created", "immutable": true})
	...
	recorder.Add("database", "webhook.producer.transaction_committed", "commit space, Event, and Delivery atomically", map[string]any{"delivery_count": 1})
