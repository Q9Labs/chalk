...
type OpsConfig struct {
	...
	WhatsAppToCritical []string
	WhatsAppToMajor    []string
...
func Load() (*Config, error) {
	...
	cfg.Ops = OpsConfig{
		IngestToken:          getEnv("OPS_INGEST_TOKEN", "ops-ingest-dev-token-change-in-production"),
		FallbackBufferBucket: getEnv("OPS_FALLBACK_BUFFER_BUCKET", ""),
		WhatsAppEnabled:      getEnvBool("OPS_WHATSAPP_ENABLED", false),
		TwilioAccountSID:     getEnv("OPS_TWILIO_ACCOUNT_SID", ""),
		TwilioAuthToken:      getEnv("OPS_TWILIO_AUTH_TOKEN", ""),
		TwilioWhatsAppFrom:   getEnv("OPS_TWILIO_WHATSAPP_FROM", ""),
		WhatsAppToCritical:   parseCommaSeparated(getEnv("OPS_WHATSAPP_TO_CRITICAL", "")),
		WhatsAppToMajor:      parseCommaSeparated(getEnv("OPS_WHATSAPP_TO_MAJOR", "")),
		PublicStatusBaseURL:  getEnv("OPS_PUBLIC_STATUS_BASE_URL", ""),
	}
...
func (c *Config) validate() error {
	...
	if c.IsProduction() && c.Ops.IngestToken == "ops-ingest-dev-token-change-in-production" {
		return fmt.Errorf("OPS_INGEST_TOKEN must be set to a secure value in production")
	}
	if c.IsProduction() && c.Ops.IngestToken == "" {
		return fmt.Errorf("OPS_INGEST_TOKEN is required in production")
	}
	if c.Ops.WhatsAppEnabled {
		if c.Ops.TwilioAccountSID == "" {
			return fmt.Errorf("OPS_TWILIO_ACCOUNT_SID is required when OPS_WHATSAPP_ENABLED=true")
		}
		if c.Ops.TwilioAuthToken == "" {
			return fmt.Errorf("OPS_TWILIO_AUTH_TOKEN is required when OPS_WHATSAPP_ENABLED=true")
		}
		if c.Ops.TwilioWhatsAppFrom == "" {
			return fmt.Errorf("OPS_TWILIO_WHATSAPP_FROM is required when OPS_WHATSAPP_ENABLED=true")
		}
	}
