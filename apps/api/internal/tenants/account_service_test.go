...
func TestAccountServiceOnboardingDefaultsManagedMediaPlaneAndPreservesReplay(t *testing.T) {
	...
	for index, record := range records {
		if record.Tenant.DefaultMediaPlane == nil || *record.Tenant.DefaultMediaPlane != "cf_sfu" {
			t.Fatalf("record %d default media plane = %v, want cf_sfu", index, record.Tenant.DefaultMediaPlane)
		}
		if !bytes.Equal(record.Tenant.MediaPlaneProviderConfig, []byte(`{"enabled":true,"provider":"cf_sfu","mode":"chalk_managed"}`)) {
