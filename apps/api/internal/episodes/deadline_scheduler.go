...
func (s *DeadlineScheduler) Run(ctx context.Context) error {
	...
	for {
		if err := s.runCycle(ctx); err != nil && ctx.Err() == nil {
			...
			slog.ErrorContext(ctx, "deadline scheduler cycle failed", "event", "episode.deadline_scheduler.failed", "error", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func (s *DeadlineScheduler) runCycle(ctx context.Context) error {
	for {
		count, err := s.repository.EnqueueDueEpisodeDeadlines(ctx, s.batch)
		if err != nil {
			return err
		}
