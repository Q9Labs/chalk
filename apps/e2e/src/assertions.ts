...
export function buildUpdatedWebhookConfig(
	existing: TenantPostMeetingWebhookConfig,
	updates: { url: string; secret: string },
): TenantPostMeetingWebhookConfig {
	const anyIncluded =
		existing.include_recording ||
		existing.include_transcript ||
		existing.include_summary ||
		existing.include_action_items;
	return {
		...existing,
		enabled: true,
		url: updates.url,
...
export function assertWebhookDelivery(opts: {
	headers: Headers;
	bodyText: string;
	payload: unknown;
	...
	roomId: string;
	recordingId: string;
	config: TenantPostMeetingWebhookConfig;
}): AssertionResult[] {
	...
	results.push(
		makeResult(
			"payload.event === meeting.recording_ready",
	...
	results.push(
		makeResult(
			"payload.meeting.id matches ROOM_ID",
			typeof meetingId === "string" && meetingId === opts.roomId,
			`got ${JSON.stringify(meetingId)}`,
		),
	);
	...
	results.push(
		makeResult("payload.meeting.started_at present", typeof startedAt === "string" && startedAt.length > 0),
	);
	...
	if (opts.config.include_recording) {
		...
		results.push(
			makeResult(
				"payload.recording.id matches RECORDING_ID",
				typeof recId === "string" && recId === opts.recordingId,
				`got ${JSON.stringify(recId)}`,
			),
		);
	...
	if (opts.config.include_transcript) {
		...
		results.push(
			makeResult(
				"payload.transcript.text non-empty",
		...
		if (opts.config.transcription?.provider) {
			results.push(
				makeResult(
					"payload.transcript.provider matches tenant",
					typeof transcript?.provider === "string" && transcript.provider === opts.config.transcription.provider,
					`got ${JSON.stringify(transcript?.provider)} expected ${JSON.stringify(opts.config.transcription.provider)}`,
				),
			);
		}
	}
	...
	if (opts.config.include_summary) {
		...
		const ok =
			...
			(typeof summary === "object" && summary != null && typeof summary.text === "string" && summary.text.length > 0);
	...
	results.push(
		makeResult(
			"header X-Chalk-Timestamp within 10 minutes",
			tsInt != null && Math.abs(nowSec - tsInt) <= 10 * 60,
			`now=${nowSec} ts=${tsInt}`,
		),
	);
...
export async function assertApiRecordingState(opts: {
	apiBaseUrl: string;
	jwt: string;
	recordingId: string;
	timeoutMs: number;
	...
	return [
		makeResult("api recording.status === ready", status === "ready", `got ${JSON.stringify(status)}`),
		makeResult(
			"api recording.storage_path set",
			typeof storagePath === "string" && storagePath.length > 0,
			`got ${JSON.stringify(storagePath)}`,
		),
	];
}
