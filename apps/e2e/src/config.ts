...
export function loadConfig(argv = process.argv.slice(2)): LoadConfigResult {
	...
	if (argv.includes("--help") || argv.includes("-h")) {
		return {
			...
			skipped: true,
			reason:
				"Usage: bun run src/index.ts [--run] [--persistent] [--timeout-ms=300000] [--webhook-port=9876] [--require]",
		};
	}
	...
	if (!runFlag) {
		return {
			...
			skipped: true,
			reason:
				"E2E harness disabled by default. Run with `--run` or set `CHALK_E2E_RUN=1` (and optionally `--require`).",
		};
	}

	const webhookPort = Number.parseInt(
		getArgValue(argv, "--webhook-port=") ?? process.env.WEBHOOK_PORT ?? "9876",
		10,
	);
	const timeoutMs = Number.parseInt(
		getArgValue(argv, "--timeout-ms=") ?? process.env.TIMEOUT_MS ?? "300000",
		10,
	);

	const webhookSecret = (process.env.WEBHOOK_SECRET?.trim() || "").length
