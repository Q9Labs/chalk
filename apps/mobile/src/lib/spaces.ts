...
async function brokerError(response: Response): Promise<BrokerError> {
  const details = brokerErrorDetails(response.status);
  return new BrokerError(response.status, details.code, details.message);
}

type BrokerErrorCode = "invalid_request" | "unauthorized" | "forbidden" | "not_found" | "conflict" | "gone" | "rate_limited" | "server_error" | "request_failed";

function brokerErrorDetails(status: number): { readonly code: BrokerErrorCode; readonly message: string } {
  switch (status) {
    case 400:
      return { code: "invalid_request", message: "The access request was invalid." };
    case 401:
      return { code: "unauthorized", message: "Access is no longer valid." };
    case 403:
      return { code: "forbidden", message: "Access is not permitted." };
    case 404:
      return { code: "not_found", message: "The requested Space was not found." };
    case 409:
      return { code: "conflict", message: "The access request conflicts with current state." };
    case 410:
      return { code: "gone", message: "This Space invite is no longer valid." };
    case 429:
      return { code: "rate_limited", message: "Too many access requests. Try again shortly." };
    default:
      return status >= 500 ? { code: "server_error", message: "Chalk could not complete the access request." } : { code: "request_failed", message: "The access request failed." };
  }
}
