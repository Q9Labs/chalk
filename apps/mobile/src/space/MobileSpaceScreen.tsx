...
export function MobileSpaceScreen({ brokerUrl, onClose, onDiagnosticsFailure, route, telemetryEnabled }: MobileSpaceScreenProps): React.JSX.Element {
  const telemetryApiBaseURLRef = useRef<string | undefined>(undefined);
  const telemetry = useMemo(
    () =>
      createMobileTelemetry({
        enabled: telemetryEnabled,
        // Authenticated journey export is supplied by the Wave 6 SDK bridge;
        // this app intentionally never derives a bearer from opaque AccessGrant data.
  ...
  if (!arrival || !client) {
    return <Entrance defaults={{ camera: true, microphone: true }} error={arrivalError} joining={isPreparing} onCancel={handleEntranceCancel} onJoin={prepareArrival} spaceName={route.spaceName ?? "Space"} />;
  }
