...
const HOOKS = [
  { id: 'room', name: 'useRoom', description: 'Join/leave, status transitions' },
  { id: 'media', name: 'useMedia', description: 'Toggle video/audio' },
  { id: 'participants', name: 'useParticipants', description: 'Local/remote lists' },
  { id: 'devices', name: 'useDevices', description: 'Enumerate cameras/mics' },
  { id: 'permissions', name: 'usePermissions', description: 'Request/check permissions' },
  { id: 'chat', name: 'useChat', description: 'Send/receive messages' },
  { id: 'recording', name: 'useRecording', description: 'Start/stop recording' },
  { id: 'screen-share', name: 'useScreenShare', description: 'Share screen' },
  { id: 'audio-routing', name: 'useAudioRouting', description: 'Speaker/earpiece/bluetooth' },
  { id: 'call-kit', name: 'useCallKit', description: 'iOS call integration' },
  { id: 'foreground-service', name: 'useForegroundService', description: 'Android background service' },
...
const COMPONENTS = [
  ...
  { id: 'screen-share-view', name: 'ScreenShareView', description: 'Screen share display' },
  { id: 'participant-tile', name: 'ParticipantTile', description: 'Participant avatar/video' },
  { id: 'video-grid', name: 'VideoGrid', description: 'Grid of participants' },
  { id: 'audio-session', name: 'AudioSession', description: 'Audio session wrapper' },
];

const E2E_FLOWS = [
  { id: 'pre-call', name: 'Pre-call Flow', description: 'Permissions, device selection, preview' },
  { id: 'full-call', name: 'Full Call Flow', description: 'Join, interact, leave' },
];
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        <View style={styles.statusSection}>
          <StatusBadge label="Connection" value={connectionStatus} color={isConnected ? 'green' : 'gray'} />
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>E2E Flows ({E2E_FLOWS.length})</Text>
          <View style={styles.grid}>
            {E2E_FLOWS.map((flow) => (
              <TouchableOpacity
                key={flow.id}
