      ...
      {transcripts.length === 0 ? (
        ...
        <Table>
          ...
          <TableBody>
            {transcripts.map((t) => (
              <>
                <TableRow key={t.id} className="cursor-pointer hover:bg-accent" onClick={() => setExpanded(expanded === t.id ? null : t.id)}>
                  ...
                  <TableRow key={`${t.id}-expanded`}>
                    <TableCell colSpan={6} className="bg-muted/50">
                      <div className="p-3 space-y-3">
                        ...
                        {!t.summary && !t.error_message && <p className="text-sm text-muted-foreground">No additional details.</p>}
                      </div>
                    </TableCell>
                  </TableRow>
                )}
