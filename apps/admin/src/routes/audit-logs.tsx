      ...
      {logs.length === 0 ? (
        ...
        <Table>
          ...
          <TableBody>
            {logs.map((log) => (
              <>
                <TableRow key={log.id} className="cursor-pointer hover:bg-accent" onClick={() => setExpanded(expanded === log.id ? null : log.id)}>
                  <TableCell>
                  ...
                  <TableCell className="text-xs">{log.resource_type ? `${log.resource_type}` : "—"}</TableCell>
