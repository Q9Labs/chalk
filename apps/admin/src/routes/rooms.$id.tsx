  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <Badge variant={room?.status === "active" ? "default" : "secondary"}>{room?.status as string}</Badge>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              <span className="text-muted-foreground">ID:</span> {room?.id}
            </div>
            <div>
              <span className="text-muted-foreground">Tenant:</span> {room?.tenant_name}
            </div>
            <div>
              <span className="text-muted-foreground">Active Participants:</span> {room?.active_participant_count}
            </div>
            <div>
              <span className="text-muted-foreground">Created:</span> {room?.created_at ? format(new Date(room.created_at as string), "PPp") : "—"}
            </div>
          </CardContent>
        </Card>
