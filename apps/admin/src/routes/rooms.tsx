  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        data={data ?? []}
        isLoading={isLoading}
        onRowClick={(row) => navigate({ to: "/rooms/$id", params: { id: row.id as string } })}
        columns={[
          {
            key: "name",
            header: "Name",
          ...
          { key: "tenant_name", header: "Tenant" },
          {
            key: "status",
            header: "Status",
            render: (r) => <Badge variant={r.status === "active" ? "default" : "secondary"}>{r.status as string}</Badge>,
          },
          {
          ...
          },
          {
            ...
            header: "Started",
            render: (r) => {
              const v = r.started_at as { Valid?: boolean; Time?: string } | undefined;
