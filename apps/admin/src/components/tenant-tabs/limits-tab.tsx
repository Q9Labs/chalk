...
export function LimitsTab({
  ...
  const mutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      api.updateTenant(tenantId, data),
    onSuccess,
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md space-y-4 pt-4">
      ...
      <div className="grid gap-2">
        ...
        <Input
          id="max_concurrent_rooms"
          name="max_concurrent_rooms"
          type="number"
          defaultValue={String(tenant.max_concurrent_rooms ?? "")}
        />
      </div>
      <div className="grid gap-2">
        <Label htmlFor="max_participants_per_room">
          Max Participants per Room
        </Label>
        <Input
          id="max_participants_per_room"
          name="max_participants_per_room"
          type="number"
          defaultValue={String(tenant.max_participants_per_room ?? "")}
        />
      </div>
      <div className="grid gap-2">
        <Label htmlFor="max_recording_duration_minutes">
          Max Recording Duration (min)
        </Label>
        <Input
          id="max_recording_duration_minutes"
          name="max_recording_duration_minutes"
          type="number"
          defaultValue={String(tenant.max_recording_duration_minutes ?? "")}
        />
      </div>
