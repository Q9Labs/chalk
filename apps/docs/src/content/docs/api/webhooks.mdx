...
```json
{
  ...
  "action_items": ["John: Finalize API documentation by Friday", "Sarah: Schedule design review for new onboarding flow", "Team: Review updated timeline in project board"],
  "errors": []
}
```
...
function verifyChalkWebhook(payload: string, signature: string, timestamp: string, secret: string): boolean {
...
function verifyChalkWebhook(payload: string, signature: string, timestamp: string): boolean {
  const age = Math.floor(Date.now() / 1000) - parseInt(timestamp, 10);
