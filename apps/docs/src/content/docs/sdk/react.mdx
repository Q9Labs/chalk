...
function App() {
  return (
    <ChalkProvider apiUrl="https://api.chalk.q9labs.ai" token={jwt}>
      <VideoConference roomId="room-123" userName="Alice" role="host" features={{ recording: true, whiteboard: true }} onEnd={(data) => console.log("Duration:", data.duration)} onLeave={() => router.push("/")} />
    </ChalkProvider>
  );
}
```
