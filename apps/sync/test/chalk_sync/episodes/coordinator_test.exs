defmodule ChalkSync.Episodes.CoordinatorTest do
  ...
  test "authoritative repair heals a completely lost fanout hint" do
    ...
    assert :ok = Coordinator.publish(identity.episode, decision.event)
  ...
  test "duplicate, delayed, and reordered head hints deliver one exact suffix" do
    ...
    Coordinator.hint(identity.episode, 3)
    Coordinator.hint(identity.episode, 2)
    Coordinator.hint(identity.episode, 3)
