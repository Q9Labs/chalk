defmodule ChalkSync.Episodes.ReducerExternalTest do
  ...
  test "admission request round-trips, rejects duplicates, and is consumed by matching admission" do
    ...
    assert {:ok, ^state} = Reducer.from_snapshot("episode-a", snapshot)
      ...
      Enum.reduce(
        ...
        fn name, current ->
          assert {:ok, event, next} =
                   Reducer.apply_external(current, name, %{"participant_id" => "owner"})
  ...
  test "external leave and end facts are unavailable through lifecycle origin" do
    ...
    assert {:ok, %{name: "episode_ended"}, ended} =
             Reducer.apply_external(state, :episode_ended, %{
  ...
  defp join_participant(state, id),
    do: join(state, id, "Grace", "observer")
