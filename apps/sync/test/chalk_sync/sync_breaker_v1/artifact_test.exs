defmodule ChalkSync.SyncBreakerV1.ArtifactTest do
  ...
  test "rejects a corrupted bounded artifact" do
    path =
      Path.join(
        System.tmp_dir!(),
        "chalk-sync-breaker-v1-corrupt-#{System.unique_integer()}.json"
      )
  ...
  test "rejects an artifact larger than one MiB" do
    path =
      Path.join(
        System.tmp_dir!(),
        "chalk-sync-breaker-v1-oversize-#{System.unique_integer()}.json"
      )
