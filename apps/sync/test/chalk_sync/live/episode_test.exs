defmodule ChalkSync.Live.EpisodeTest do
  ...
  defmodule Authority do
    def participant_authority(_episode, participant_id, generation) do
      send(controller(), {:participant_authority, participant_id, generation})
    ...
    def complete_publication_grant(_episode, reservation_id, outcome) do
  ...
  test "reserves publication authority before provider grant and completes afterward", %{
    ...
    assert {_state, %{"outcome" => "confirmed"}} =
             Episode.live_target(Episode.new(identity.episode), identity, target)
    ...
    assert [
             {:grant_publication, "camera-grant-order-01", [^episode, ^participant_id, :camera]}
  ...
  test "ambiguous grant converges to confirmed with the same operation id", %{adapter: adapter} do
    ...
    assert {state, %{"outcome" => "retryable_failure"}} =
             Episode.live_target(state, identity, target(operation_id))
  ...
  test "provider bridge disable and enable requests include the live authority generation" do
    ...
    assert {_state, %{"outcome" => "confirmed"}} =
             Episode.live_target(state, identity, target("camera-generation-grant-01"))
  ...
  test "blocking and raising live provider callbacks return within the configured bound" do
    ...
    assert {_state, %{"outcome" => "retryable_failure"}} =
             Episode.live_target(state, identity, target("camera-blocked-grant-01"))
    ...
    assert {:error, :dependency_unavailable} = Episode.register(state, identity, self())
