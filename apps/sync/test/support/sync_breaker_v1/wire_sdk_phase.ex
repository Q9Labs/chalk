defmodule ChalkSync.SyncBreakerV1.WireSdkPhase do
  @moduledoc false
  ...
  defp exercise_wire do
    target_observations =
      ...
      |> Enum.map(fn {{name, payload}, index} ->
        ...
        {:ok, {:command, decoded}} = ProtocolV1.decode(JSON.encode!(frame))
        ...
        encoded = ProtocolV1.encode!(ack)
        true = GeneratedV1.valid_server_frame?(JSON.decode!(encoded))
  ...
  defp invalid_wire_shapes do
    ...
    results =
      Enum.map(shapes, fn {label, frame} ->
        decoded_rejected = match?({:error, _reason}, ProtocolV1.decode(JSON.encode!(frame)))
