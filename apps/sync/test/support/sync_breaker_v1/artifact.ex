defmodule ChalkSync.SyncBreakerV1.Artifact do
  @moduledoc false

  @schema "chalk.sync.breaker.v1"
  @max_bytes 1_048_576
  ...
  def read!(path) do
    ...
    unless envelope["sha256"] == checksum(payload) and payload["artifact_schema"] == @schema do
      raise ArgumentError, "invalid or corrupt SyncEngine v1 breaker artifact"
    end
  ...
  defp ensure_bounded!(_bytes),
    do: raise(ArgumentError, "SyncEngine v1 breaker artifact exceeds the 1 MiB bound")
end
