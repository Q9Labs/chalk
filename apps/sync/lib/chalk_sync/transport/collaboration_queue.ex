defmodule ChalkSync.Transport.CollaborationQueue do
  @moduledoc """
  Bounded logical queue for space-action frames on a Sync v1 socket.

  The owning socket process calls this queue directly. `take/2` enforces the
  frozen space-action burst before yielding back to pending control traffic.
  """
  ...
  def new do
    ...
    :ets.insert(
      state,
      {:state, %{frames: 0, bytes: 0, next_sequence: 1, collaboration_burst: 0}}
    )
  ...
  def take(queue, control_pending? \\ false) when is_boolean(control_pending?) do
    with :ok <- owner_open?(queue),
      ...
      cond do
        ...
        control_pending? and state.collaboration_burst >= @burst_limit ->
          :control_required
        ...
        true ->
          ...
          put_state(queue, %{
            ...
            | frames: state.frames - 1,
              bytes: state.bytes - byte_size(encoded),
              collaboration_burst: min(state.collaboration_burst + 1, @burst_limit)
          })
  ...
  def control_checked(queue) do
    with :ok <- owner_open?(queue),
         {:ok, state} <- queue_state(queue) do
      put_state(queue, %{state | collaboration_burst: 0})
      :ok
    end
  end
    ...
    with :ok <- owner_open?(queue),
      ...
      {:ok,
       %{
         ...
         burst: min(state.collaboration_burst, @burst_limit)
       }}
    end
  end
