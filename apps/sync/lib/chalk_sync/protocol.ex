defmodule ChalkSync.ProtocolV1 do
  @moduledoc "Strict protocol-v1 framing at the WebSocket boundary."
  ...
  def decode(text)
    ...
    with {:ok, %{} = frame} <- JSON.decode(text),
         {:ok, decoded} <- GeneratedV1.decode_client_frame(frame),
  ...
  def encode!(frame) do
    if GeneratedV1.valid_server_frame?(frame),
      do: JSON.encode!(frame),
      else: raise(ArgumentError, "invalid protocol-v1 server frame")
  end
  ...
  defp event_frame(event) do
    ...
    case Enum.filter(origins, fn {_key, value} -> is_binary(value) end) do
      ...
      _ -> raise ArgumentError, "protocol-v1 event must have exactly one durable origin"
    end
  end
