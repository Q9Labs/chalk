defmodule ChalkSync.RoomActions.Fanout.PostgresNotifications do
  @moduledoc """
  ...
  @head_channel "chalk_room_action_heads"
  @transient_channel "chalk_room_action_transient"
  ...
  @impl true
  def publish_chat_head(_adapter, %SessionKey{} = session, head) do
    payload =
      encode_payload(%{
        "kind" => "chat_head",
    ...
    notify(session, @head_channel, payload)
  end
  ...
  defp publish_transient(session, kind, event) do
    ...
    if byte_size(payload) <= @transient_payload_bytes,
      do: notify(session, @transient_channel, payload),
  ...
  def decode_notification(channel, payload)
    ...
    with {:ok, frame} <- JSON.decode(payload),
         {:ok, session} <- decode_session(frame),
         {:ok, kind, value} <- decode_value(channel, frame) do
      {:ok, {kind, session, value}}
    else
  ...
  defp decode_session(%{
    ...
    if Enum.all?([tenant_id, room_id, session_id], &(is_binary(&1) and &1 =~ @uuid)) do
      {:ok, %SessionKey{tenant_id: tenant_id, room_id: room_id, session_id: session_id}}
    else
      {:error, :invalid_payload}
    end
  end

  defp decode_session(_frame), do: {:error, :invalid_payload}
  ...
  defp notify(session, channel, payload) do
    case Postgrex.query(
