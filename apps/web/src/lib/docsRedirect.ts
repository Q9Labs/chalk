...
export function getDocsExternalUrl(pathname: string, search = "", hash = ""): string {
  const normalizedPathname = pathname.startsWith("/documentation") ? pathname.slice("/documentation".length) : pathname.startsWith("/docs") ? pathname.slice("/docs".length) : pathname;
