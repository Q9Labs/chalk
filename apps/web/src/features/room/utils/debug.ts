...
class RoomDebugger {
	...
	constructor() {
		...
		if (typeof window !== "undefined") {
			...
			this.verboseMode = urlParams.get("verbose") === "true" ||
				localStorage.getItem("chalk_debug_verbose") === "true";
		}
	}
	...
	stateChange<T>(stateName: string, prevValue: T, newValue: T) {
		...
		console.log(
			`%c${prefix} 🔄 STATE: ${stateName} changed`,
			COLORS.state,
	...
	render(reason?: string) {
		...
		console.log(
			`%c${prefix} 🎨 RENDER #${this.renderCount}${reasonStr} [+${timeSinceLastRender}ms]`,
			COLORS.render
		);
	}
	...
	summary(data: {
		...
		const media = data.mediaState
			? `[${data.mediaState.video ? "📹" : "📷"}${data.mediaState.audio ? "🎤" : "🔇"}${data.mediaState.screen ? "🖥️" : ""}]`
			: "";
		const panel = data.activePanel ? `[${data.activePanel}]` : "";

		console.log(
			`%c${prefix} ${status} SUMMARY: Room=${data.roomId} | ${data.participants || 0} participants | ${media} ${panel}`,
			COLORS.state
		);
	}
}
