...
export function CodeBlock({ children, language = "typescript", filename }: CodeBlockProps) {
  ...
  return (
    <div className="my-6 rounded-lg border border-border overflow-hidden bg-muted/50">
      {filename && <div className="px-4 py-2 border-b border-border bg-muted/80 text-sm text-muted-foreground font-mono">{filename}</div>}
      <div className="relative group">
        <button type="button" onClick={handleCopy} className="absolute right-2 top-2 p-2 rounded-md bg-background/80 hover:bg-background border border-border opacity-0 group-hover:opacity-100 transition-opacity" aria-label={copied ? "Copied!" : "Copy code"}>
