  ...
  return (
    <div className="font-app min-h-screen bg-white dark:bg-[#030303] text-zinc-900 dark:text-zinc-100 antialiased tracking-tight">
        ...
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-6">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-4 text-sm font-medium tracking-tight">
              <span className="text-zinc-900 dark:text-zinc-100 font-semibold">{doc.title}</span>
            </nav>
          </div>
          <div className="flex items-center gap-3 pl-2 md:pl-4 md:border-l border-zinc-200/50 dark:border-white/[0.06]">
            <Link to={otherHref} className="text-xs font-medium text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors">
              {otherLabel}
            </Link>
            <div className="w-px h-4 bg-zinc-200 dark:bg-white/[0.08]" />
            <button type="button" onClick={toggleTheme} className="p-2 text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 rounded-full hover:bg-zinc-50 dark:hover:bg-white/[0.04] transition-colors" aria-label="Toggle theme">
      ...
      <main className="mx-auto max-w-3xl px-6 py-12 md:py-20">
        {/* Title block */}
        <div className="mb-10">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">{doc.title}</h1>
          <p className="mt-2 text-sm text-zinc-400 dark:text-zinc-500">Last updated: {doc.lastUpdated}</p>
        </div>
        ...
        <nav className="mb-12 rounded-2xl border border-zinc-200/80 dark:border-white/[0.08] bg-zinc-50/50 dark:bg-white/[0.02] px-6 py-5" aria-label="Table of contents">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-3">Contents</h2>
          <ol className="space-y-2">
            {doc.sections.map((section, i) => {
              ...
              return (
                <li key={id}>
                  <a href={`#${id}`} className={`group flex items-center gap-2 text-sm transition-colors ${isActive ? "text-zinc-900 dark:text-zinc-100 font-medium" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300"}`}>
                    <span className={`tabular-nums text-[11px] ${isActive ? "text-zinc-400 dark:text-zinc-500" : "text-zinc-300 dark:text-zinc-600"}`}>{i + 1}.</span>
                    {section.title}
                  </a>
                </li>
              );
          ...
          {doc.sections.map((section) => {
            ...
            return (
              <section key={id} id={id} className="scroll-mt-24">
                <h2 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 mb-4">{section.title}</h2>
                <div
        ...
        <div className="mt-16 pt-8 border-t border-zinc-200/80 dark:border-white/[0.06]">
          <p className="text-sm text-zinc-400 dark:text-zinc-500">
            Also see our{" "}
            <Link to={otherHref} className="text-zinc-700 dark:text-zinc-300 underline underline-offset-2 decoration-zinc-300 dark:decoration-zinc-600 hover:decoration-zinc-500 dark:hover:decoration-zinc-400 transition-colors">
              {otherLabel}
            </Link>
            .
          </p>
