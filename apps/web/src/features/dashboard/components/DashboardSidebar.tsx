...
export function DashboardSidebar({
  meetings, search, onSearchChange, selectedId, onSelectMeeting,
  sdkClient, scheduledRooms, roomsLoading, roomsError, onRefreshRooms
      ...
      <div className="p-6 space-y-8">
        ...
        <div className="relative w-full group">
          ...
          <Input 
            ...
            className="h-10 bg-background border-border/50 pl-10 rounded-xl shadow-sm focus-visible:ring-primary/20 text-foreground placeholder:text-muted-foreground" 
            value={search} 
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="font-mono text-[10px] px-2 py-0.5 rounded-full bg-secondary border border-border/50 text-muted-foreground font-bold">
            ...
            {filteredMeetings.map((m) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left p-4 rounded-xl transition-all outline-none focus-visible:ring-2 focus-visible:ring-primary group border",
                    isActive 
                ...
                >
                  <div className="flex flex-col gap-2 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h4 className={cn("font-semibold text-sm leading-tight truncate flex-1", isActive ? "text-foreground" : "text-foreground/80 group-hover:text-foreground")}>
                        {m.room_name || `Untitled Room`}
                      </h4>
                      {m.status === "ready" && (
                        <div className={cn("w-1.5 h-1.5 rounded-full shrink-0 mt-1", isActive ? "bg-primary" : "bg-muted-foreground/30")} />
                      )}
                    </div>
