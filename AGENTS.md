...
- Primary brand color: `packages/sdk-react/src/styles/variables.css` (`--chalk-brand`, `--chalk-brand-hover`); this drives SDK components.
- Web app theme tokens: `apps/web/src/styles.css` (`--primary`, `--sidebar-primary`, etc.) for landing + app chrome.
- Meeting room in demo app uses Tailwind theme tokens (`bg-background`, `text-foreground`, `border-border`) in `apps/web/src/features/room/components/**` and `apps/web/src/routes/room/$roomId.tsx`.
- Light/dark: toggle root class (`html.dark` / `html.light`) in `apps/web/src/routes/__root.tsx`; SDK respects `data-chalk-theme`.
