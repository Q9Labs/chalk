...
**A sound set is a voice; the grammar is the language.** Every set pronounces the _same_ musical gestures: join always rises, leave always falls, and error always settles low. Each set has its own timbre, key, and space. The family resemblance is structural (intervals, rhythm, envelope logic), not just a shared reverb preset. A user can switch sets and lose nothing because they already speak the language.

**The enemy is annoyance, and annoyance is a function of repetition.** A sound heard once per Episode can afford character. A sound heard forty times must approach transparency. Loudness, length, and personality are budgeted by _frequency of occurrence_, not importance.
...
- Concrete sound-design vocabulary beats poetry: name the instrument, the register, the space ("soft felt piano", "dry, no reverb", "small warm space").
- Musical descriptors work: keys, intervals, and chords ("in F major", "rising a perfect fifth", "add9 chord"). Pair every interval with plain-audio language ("two ascending notes, low to high") as insurance.
- Sequence multi-part sounds with **"then"**: "three rising notes, then a soft held confirmation tone."
- State positives, never negations. Use "gentle, calm, soft" rather than "not harsh, not annoying."
- Keep prompts ~15–35 words; detail sharpens, bloat dilutes.

**Workflow per sound:** generate 4–6 takes, audition against the set's existing keepers (does it sound like the same instrument in the same space?), pick one, post-process it, and run the QA gauntlet.

---

## Global rules (every set obeys these)

1. **Duration budget** (final trimmed asset, not generation length): tier-1 ≤ 150ms of gesture + natural tail, tier-2 ≤ 400ms, tier-3 ≤ 800ms. One exception: _episode-end_ may run ~1.2s, the only lyrical moment a set gets.
...
5. **Coalescing.** ≥3 joins in 5s → one join sound. Messages: max one sound per 3s per Space. ≥5 reactions in 2s → single _applause_ swell instead of five pops.
6. **Self vs. remote.** Your own actions play −4 dB quieter and drier because you already have visual confirmation.
7. **Sound is never the only signal** (a11y, muted tabs). Each event must be identifiable blind: distinct in mono and at low volume.
8. **Delivery:** 48 kHz/24-bit WAV masters, OGG + AAC delivery, < 50 KB per file. Naming is `sounds/<set>/<event>.<ext>`. The vocabulary is the contract; sets are interchangeable implementations.
...
### Tier 1: many times per Episode (near-transparent)
...
### Tier 2: a few times per Episode (quietly characterful)
...
### Tier 3: once per Episode or rarer (full character allowed)
...
> `Soft felt piano and warm wooden mallet percussion, matte and intimate, in F major pentatonic, recorded close in a small warm space.`

The house voice is warm and a little schoolhouse-poetic, like the product name. **Signature detail:** on `click` and `join`, append `with a faint dusty chalk-tap texture at the attack`.
...
For sound-sensitive people and open offices; rendered ~4 dB under global normal. Pitch words in gestures read as filter sweeps here, and that is intended. **Signature detail:** `join` / `leave` become `a single soft page turning forward` / `a single soft page turning backward`.
...
**Signature detail:** `recording-start` / `recording-stop` prepend `a soft mechanical tape-deck button ka-chunk, then`. That is the set's thesis.

### 5. Kalimba

> `Intimate kalimba and music box, close-miked plucked metal tines, slightly detuned pairs, in G major, tiny and warm.`

**Signature detail:** `episode-end` becomes `a four-note music box phrase winding gently down to rest, like a lid softly closing`.
...
The evening set gives every attack a bloom, never a strike. Trim blooms hard in post to hold tier budgets. **Signature detail:** its `connection-lost` / `connection-restored` pair is the set's best moment. Keep the light-dimming language and add `like a pad losing power` / `warming back to full brightness`.
...
The minimalist favors function over feeling. Everything is trimmed < 150ms in post regardless of tier. **Signature detail:** none. Restraint is the signature.

### 9. Hearth

> `Soft breathy ocarina and low wooden flute, warm human breath in the tone, in B-flat major, recorded in a small wooden space, folk warmth.`

The set that feels most _played by a person_. **Signature detail:** `hand-raise` appends `a genuinely questioning little blown lift, almost vocal`.
...
The modern-product voice sits in Slack/Notion territory but warmer. **Signature detail:** `admitted` appends `completing the chord that the knock began`. Getting through the Entrance audibly finishes the thought.
...
```
Soft felt piano and warm wooden mallet percussion, matte and intimate, in F major pentatonic, recorded close in a small warm space. UI notification sound: two soft ascending notes rising a perfect fifth, low to high, open and welcoming, with a faint dusty chalk-tap texture at the attack. One-shot, gentle and pleasant, high-quality professionally designed app notification sound, clean and minimal.
```

→ `join`, `duration_seconds: 1.0`, `prompt_influence: 0.7`

```
Soft felt piano and warm wooden mallet percussion, matte and intimate, in F major pentatonic, recorded close in a small warm space. UI notification sound: two soft notes falling a minor third and settling on a stable low tone, apologetic, calm and kind, never harsh. One-shot, gentle and pleasant, high-quality professionally designed app notification sound, clean and minimal.
```
...
2. **The laptop test:** full vocabulary on built-in MacBook speakers at 50% volume. Everything should be audible and nothing shrill.
...
5. **The mono test:** full set summed to mono. There must be no phase-cancelled ghosts, and all events must stay distinct.
6. **The midnight test:** full set at low volume in a quiet space. Nothing startles.

## Choosing

**Default:** Chalk. **Quiet / sensory-sensitive:** Paper or Signal. **Late-night:** Aurora. Selection is per-user, per-device; tenant owners pick the tenant default. The preview UI plays a fixed demo reel (join → message → hand-raise → reaction → leave) so sets are compared on identical material.
