...
colors:
  ...
  on-surface: "#0C0E12"
  on-surface-variant: "#555B65"
  primary: "#0C0E12"
  on-primary: "#FFFFFF"
  secondary: "#55AAC9"
  on-secondary: "#0C0E12"
  outline: "#C9C8C2"
  outline-variant: "#DEDDD7"
  error: "#B94C4C"
---
...
Tokens and composed themes live in [shared UI styles](packages/ui/src/styles/index.css), [web theme](apps/web/src/lib/theme.ts), and SDK theme modules. The frontmatter describes the default light palette; named palette/texture options remain supported. This file owns visual direction, not delivery status. It is not a visual QA signoff.

## 1. Visual Theme & Atmosphere

Chalk should feel clear, warm, and quietly confident. It is a realtime work surface, so the design must help people understand the Space at a glance and act without hesitation.
...
Space details contains the Space name, Episode duration, join link, security note, and connection information. Settings uses a left navigation on desktop and horizontal tabs on mobile. Close actions remain in the top-right, and the initial focus moves inside the dialog.
...
| Circle        | `999px` | Avatars and round Space controls only |
...
Icons use a simple rounded-outline family with consistent optical weight. Standard sizes are `15px`, `18px`, and `20px`; large Space controls may use `22px` to `24px`.
...
The stage has `12px` tile gaps and outer padding from `12px` on mobile to `32px` on desktop. Opening Participants or Chat creates a `340px` second column at desktop sizes. The header shows the Space name, duration, information action, and subtle layout selector.
...
Use “warm porcelain canvas,” “black decisive media controls,” “soft purposeful Chalk colors,” and “a calm shared Space.” Preserve Figtree interface text, Bricolage Grotesque display type, and Spline Sans Mono metadata.

### Component prompts

Design Entrance with honest device and admission states, then a live Space with recognizable Participants and quiet collaboration panels. Use the same capability-driven controls in embedded and standalone layouts.
