...
export interface Link01IconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const Link01Icon = forwardRef<Link01IconHandle, Link01IconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
  ...
  return (
    <div className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }} data-hugeicons-animated="true">
      <svg onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} className="h-full w-full" xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="visible">
        <motion.g variants={linkVariants} animate={controls} initial="normal" style={{ transformOrigin: "12px 12px" }}>
          <path
