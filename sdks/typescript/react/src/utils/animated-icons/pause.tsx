...
export interface PauseIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const pauseBarVariants: Variants = {
  ...
  animate: (i: number) => {
    ...
    return {
      transform: ["translate(0px, 0px) scaleY(1)", `translate(${direction * 0.8}px, 0.65px) scaleY(0.91)`, `translate(${direction * -0.55}px, -0.35px) scaleY(1.045)`, `translate(${direction * 0.14}px, 0px) scaleY(0.99)`, "translate(0px, 0px) scaleY(1)"],
      transition: {
...
const PauseIcon = forwardRef<PauseIconHandle, PauseIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
