...
export interface PinIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const pinVariants: Variants = {
  ...
  animate: {
    transform: ["translate(0px, 0px) rotate(0deg)", "translate(-0.8px, 0.8px) rotate(-2deg)", "translate(0.3px, -0.3px) rotate(0.8deg)", "translate(0px, 0px) rotate(0deg)"],
    transition: { duration: 0.5, ease: [0.23, 1, 0.32, 1] },
  },
};

const PinIcon = forwardRef<PinIconHandle, PinIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
  ...
  return (
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="visible">
        <motion.g variants={pinVariants} animate={controls} initial="normal" style={{ transformOrigin: "12px 12px" }}>
          <path d="M3 21L8 16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" />
          <path
