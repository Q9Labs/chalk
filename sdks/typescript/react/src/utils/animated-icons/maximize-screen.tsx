...
export interface MaximizeScreenIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}

const MAXIMIZE_WINDOW = "M2 17C2 15.1144 2 14.1716 2.58579 13.5858C3.17157 13 4.11438 13 6 13H7C8.88562 13 9.82843 13 10.4142 13.5858C11 14.1716 11 15.1144 11 17V18C11 19.8856 11 20.8284 10.4142 21.4142C9.82843 22 8.88562 22 7 22H6C4.11438 22 3.17157 22 2.58579 21.4142C2 20.8284 2 19.8856 2 18V17Z";
...
function createExpandedWindowPath({ right, top }: { right: number; top: number }) {
...
const sourceWindowVariants: Variants = {
  ...
  animate: {
    ...
    strokeDasharray: ["100 0", "100 0", "100 0", "100 0", "100 0", "100 0", "36 0.4", "12 1", "5 1.8", "2.6 2.2", "5 1.8", "36 0.4", "100 0"],
    strokeDashoffset: [0, 0, 0, 0, 0, 0, 0.5, 1.5, 3, 4, 3, 0.5, 0],
    visibility: ["visible", "visible", "visible", "visible", "visible", "visible", "visible", "visible", "visible", "visible", "hidden", "hidden", "visible"],
    transition: {
      duration: MAXIMIZE_DURATION,
      ease: "linear",
      times: MAXIMIZE_TIMES,
...
const MaximizeScreenIcon = forwardRef<MaximizeScreenIconHandle, MaximizeScreenIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
  ...
  return (
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="hidden">
        <defs>
          <mask id={occlusionMaskId} x="0" y="0" width="24" height="24" maskUnits="userSpaceOnUse">
            ...
            <motion.g variants={generatedGeometryVariants} animate={controls} initial="normal">
              <motion.path d={MAXIMIZE_WINDOW} fill="black" stroke="black" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" variants={occlusionMaskVariants} animate={controls} initial="normal" />
            </motion.g>
          </mask>
        </defs>
        <motion.path
        ...
        />
        <motion.path d={MAXIMIZE_WINDOW} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" mask={`url(#${occlusionMaskId})`} variants={sourceWindowVariants} animate={controls} initial="normal" />
        <motion.path d={MAXIMIZE_OUTER_SOURCE} stroke="currentColor" strokeLinecap="round" strokeWidth="inherit" variants={sourceOuterFrameVariants} animate={controls} initial="normal" />
        <motion.g variants={generatedGeometryVariants} animate={controls} initial="normal">
          <motion.path d={MAXIMIZE_WINDOW} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" variants={arrivingWindowVariants} animate={controls} initial="normal" />
        </motion.g>
      </svg>
    </div>
  );
