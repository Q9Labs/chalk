...
export interface UserGroupIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const sideUserVariants: Variants = {
  ...
  animate: (direction: number) => ({
    transform: [`translateX(${direction * 0.9}px)`, `translateX(${direction * -0.2}px)`, "translateX(0px)"],
    transition: { duration: 0.5, ease: [0.23, 1, 0.32, 1] },
  }),
};

const UserGroupIcon = forwardRef<UserGroupIconHandle, UserGroupIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
    ...
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="visible">
        ...
        <motion.path
          ...
          initial="normal"
        />
        <motion.path d="M22 16.5C22 13.7386 19.5376 11.5 16.5 11.5" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" variants={sideUserVariants} custom={1} animate={controls} initial="normal" />
        <motion.path
          ...
          initial="normal"
        />
        <motion.path d="M7.5 11.5C4.46243 11.5 2 13.7386 2 16.5" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" variants={sideUserVariants} custom={-1} animate={controls} initial="normal" />
      </svg>
    </div>
  );
});
