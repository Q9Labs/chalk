...
export interface Moon02IconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const starVariants: Variants = {
  ...
  animate: (i: number) => ({
    ...
    transform: ["scale(0.55)", "scale(1)", "scale(0.78)", "scale(1)", "scale(0.55)"],
    transition: {
      duration: 1.3,
      ease: [0.77, 0, 0.175, 1],
      repeat: Infinity,
...
const Moon02Icon = forwardRef<Moon02IconHandle, Moon02IconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
    ...
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="visible">
        ...
        <motion.g variants={generatedGeometryVariants} animate={controls} initial="normal">
          <motion.path d="M17.5 4.2V6.2M16.5 5.2H18.5" stroke="currentColor" strokeLinecap="round" strokeWidth="1" variants={starVariants} custom={0} animate={controls} initial="normal" style={{ transformOrigin: "17.5px 5.2px" }} />
          ...
          <motion.path d="M19.3 8.25V9.75M18.55 9H20.05" stroke="currentColor" strokeLinecap="round" strokeWidth="0.9" variants={starVariants} custom={2} animate={controls} initial="normal" style={{ transformOrigin: "19.3px 9px" }} />
        </motion.g>
      </svg>
    </div>
  );
