...
export interface InformationCircleIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}
...
const InformationCircleIcon = forwardRef<InformationCircleIconHandle, InformationCircleIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
  ...
  return (
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="visible">
        <motion.circle cx="12" cy="12" r="10" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" variants={circleVariants} style={{ transformOrigin: "12px 12px" }} animate={controls} initial="normal" />
        <motion.path d="M12 12V16" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" variants={stemVariants} style={{ transformOrigin: "12px 16px" }} animate={controls} initial="normal" />
        <motion.path
