...
export interface SentIconProps extends SVGAttributes<SVGSVGElement> {
  size?: number | string;
}

const SENT_PLANE = "M21.0477 3.05293C18.8697 0.707363 2.48648 6.4532 2.50001 8.551C2.51535 10.9299 8.89809 11.6617 10.6672 12.1581C11.7311 12.4565 12.016 12.7625 12.2613 13.8781C13.3723 18.9305 13.9301 21.4435 15.2014 21.4996C17.2278 21.5892 23.1733 5.342 21.0477 3.05293Z";

const SENT_DURATION = 0.92;
const SENT_TIMES = [0, 0.07, 0.14, 0.22, 0.32, 0.43, 0.5, 0.54, 0.62, 0.71, 0.8, 0.88, 0.95, 1];
...
function PlaneGeometry() {
  return (
    <>
      <path d={SENT_PLANE} stroke="currentColor" strokeWidth="inherit" />
      <path d="M11.4999 12.5L14.9999 9" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="inherit" />
    </>
  );
}

const SentIcon = forwardRef<SentIconHandle, SentIconProps>(({ onMouseEnter, onMouseLeave, className, size = 24, ...props }, ref) => {
    ...
    <div data-hugeicons-animated="true" className={cn("inline-flex", className)} style={{ strokeWidth: 1.5 }}>
      <svg className="h-full w-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" overflow="hidden">
        <motion.g variants={generatedGeometryVariants} animate={controls} initial="normal">
          <motion.path d="M7.2 14.8L2.2 19.8" stroke="currentColor" strokeLinecap="round" strokeWidth="inherit" variants={trailVariants} custom={0} animate={controls} initial="normal" />
          <motion.path d="M10.2 17.6L6.6 21.2" stroke="currentColor" strokeLinecap="round" strokeWidth="inherit" variants={trailVariants} custom={1} animate={controls} initial="normal" />
          <motion.path d="M5.2 12.6L2 15.8" stroke="currentColor" strokeLinecap="round" strokeWidth="inherit" variants={trailVariants} custom={2} animate={controls} initial="normal" />
        </motion.g>
