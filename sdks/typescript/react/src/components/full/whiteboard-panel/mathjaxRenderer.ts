...
async function loadMathJax(): Promise<MathJaxRuntime> {
  ...
  mathJaxReady = (async () => {
    if (typeof document === "undefined") {
      throw new Error("Math rendering is only available in the browser");
    }
    ...
    target.MathJax = {
      ...(existing ?? {}),
      options: {
        ...(existing?.options as object | undefined),
        enableMenu: false,
      },
      startup: {
        ...(existing?.startup as object | undefined),
        typeset: false,
      },
      svg: {
        fontCache: "none",
        ...(existing?.svg as object | undefined),
      },
...
export function getRenderedSvgSize(svg: string): { width: number; height: number } {
  ...
  const width = widthAttr?.endsWith("ex") ? parseSvgLength(widthAttr, MATHJAX_INPUT_EX) : parseSvgLength(widthAttr, 1);
  const height = heightAttr?.endsWith("ex") ? parseSvgLength(heightAttr, MATHJAX_INPUT_EX) : parseSvgLength(heightAttr, 1);
  ...
  if (typeof viewBoxWidth === "number" && typeof viewBoxHeight === "number" && Number.isFinite(viewBoxWidth) && Number.isFinite(viewBoxHeight) && viewBoxWidth > 0 && viewBoxHeight > 0) {
    const normalizedWidth = viewBoxWidth;
    const normalizedHeight = viewBoxHeight;
    return {
      width: Math.max(64, Math.ceil(normalizedWidth / 40 + 24)),
      height: Math.max(40, Math.ceil(normalizedHeight / 40 + 24)),
    };
  }
