      ...
      return (
        <div className={cn("flex flex-col gap-2 mt-2", isLocal ? "items-end" : "items-start")}>
          {attachments.map((file) => {
            ...
            if (isImage && imageUrl) {
              return (
                <button type="button" key={file.attachmentId} onClick={() => void handleAttachmentClick(file)} className="relative group overflow-hidden rounded-lg border border-border/50 max-w-full sm:max-w-xs aspect-auto" aria-label={`Download ${file.fileName}`}>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={cn("text-[11px]", isLocal ? "text-primary-foreground/70" : "text-muted-foreground")}>{formatFileSize(file.byteLength)}</p>
                </div>
              </button>
            );
          })}
    ...
    const renderStatus = () => {
      ...
      return (
        <div className="flex items-center gap-1 group/status relative" title={statusTitle}>
          {status === "pending" ? <div className="w-3 h-3 rounded-full animate-spin border-2 border-muted-foreground/40 border-t-transparent" /> : isRead ? <TickDouble01Icon className="w-3.5 h-3.5 text-primary" /> : <Tick01Icon className="w-3.5 h-3.5 text-muted-foreground" />}
          <span className={cn("text-[11px]", isRead ? "text-primary" : "text-muted-foreground")}>{statusLabel}</span>

          {isLocal && readByCount > 0 && <div className="invisible group-hover/status:visible absolute bottom-full right-0 mb-2 whitespace-nowrap bg-popover text-popover-foreground px-2 py-1 rounded text-[10px] shadow-lg border border-border z-10">Read by: {readers.join(", ")}</div>}
        </div>
      );
    };
