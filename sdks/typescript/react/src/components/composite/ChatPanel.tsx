...
import type { ChatAttachment, ChatMessage, ChatReadReceipt, ChatSendInput, SpaceSnapshot } from "@q9labsai/chalk-client";
...
export const ChatPanel = React.memo(
  ({
    ...
    return (
      <div className={cn("relative flex h-full min-h-0 w-full flex-col overflow-hidden bg-transparent text-[var(--chalk-text)]", variant !== "mobile" && "animate-in slide-in-from-right-5 duration-300", className)} role="complementary" aria-label="Chat panel">
        {variant === "sidebar" ? (
          <header className="chalk-textured-surface flex items-center justify-between border-b border-[var(--chalk-line)] bg-[var(--chalk-surface)] px-5 py-[18px]">
            <h2 className="text-xl font-semibold tracking-[-0.025em] text-[var(--chalk-text)]">{title}</h2>
            {onClose ? (
              <button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full border border-[var(--chalk-line)] text-[var(--chalk-muted-text)] transition-colors hover:bg-[var(--chalk-stage)] hover:text-[var(--chalk-text)]" aria-label="Close chat">
        ...
        <div ref={scrollRef} className="chalk-textured-surface flex-1 overflow-y-auto bg-[var(--chalk-surface)] px-2 py-5" aria-label="Chat messages" aria-live="polite" onScroll={handleScroll}>
          ...
          {messages.length === 0 && pendingMessages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center px-6 text-center">
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--chalk-stage)] text-[var(--chalk-accent)]">
                <Message01Icon className="h-8 w-8" />
              </div>
              <h3 className="mb-1 font-medium">No messages yet</h3>
              <p className="max-w-[220px] text-sm text-[var(--chalk-muted-text)]">Send a message to start the conversation.</p>
            </div>
          ) : (
            grouped.map((group) => (
          ...
          {pendingMessages.map((pending) => (
            <div key={pending.clientMessageId} className="my-2">
              ...
              {pending.status === "failed" ? (
                <div className="mr-14 flex items-center justify-end gap-2 text-xs text-[var(--chalk-muted-text)]">
                  <span>{pending.error?.message || "Not sent"}</span>
                  {onRetryMessage ? (
                    <button type="button" className="font-medium text-[var(--chalk-accent)] hover:underline" onClick={() => void onRetryMessage(pending.clientMessageId)}>
                      Retry
                    </button>
                  ) : null}
                </div>
        ...
        {(error || composerError) && (
          <p role="alert" className="mx-4 mb-2 rounded-lg bg-[var(--chalk-danger-surface)] px-3 py-2 text-sm text-[var(--chalk-danger)]">
            {composerError || error}
          </p>
        )}
        {stagedFiles.length > 0 ? (
          <div className="flex flex-wrap gap-2 border-t border-[var(--chalk-line)] px-4 pt-3" aria-label="Attachments">
            {stagedFiles.map((file, index) => (
              <div key={`${file.name}-${file.size}-${file.lastModified}-${index}`} className="flex max-w-full items-center gap-2 rounded-full bg-[var(--chalk-stage)] px-3 py-1.5 text-xs">
        ...
        <div className="chalk-textured-surface flex items-end gap-2 border-t border-[var(--chalk-line)] bg-[var(--chalk-surface)] px-4 py-4">
          {onUploadAttachment ? (
            <>
          ...
          <textarea
            ...
            aria-label="Message"
            className="min-h-11 max-h-[120px] flex-1 resize-none rounded-[8px] border border-[var(--chalk-line)] bg-[var(--chalk-surface)] px-3.5 py-3 text-sm text-[var(--chalk-text)] outline-none transition placeholder:text-[var(--chalk-muted-text)] focus:border-[var(--chalk-line)]"
            rows={1}
          />
          <Button
            type="button"
            size="icon"
            className="h-11 w-11 shrink-0 rounded-[8px] bg-[var(--chalk-accent)] !text-[var(--chalk-accent-text)] hover:bg-[var(--chalk-accent)]"
