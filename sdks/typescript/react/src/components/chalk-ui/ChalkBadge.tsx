...
import { classicToneSurfaceClasses, TONE_FILLS, TONE_STROKES, type ChalkSeed, type ChalkTone } from "./common";
...
export const ChalkBadge = forwardRef<HTMLSpanElement, ChalkBadgeProps>(function ChalkBadge({ children, className, tone = "neutral", seed, roughness, dot = false, count, max = 99, showZero = false, ...props }, ref) {
  ...
  return (
    <span
      ...
      className={cn(
        skin === "classic"
          ? cn(classicToneSurfaceClasses(tone), "inline-flex min-h-6 min-w-6 items-center justify-center rounded-full px-2 text-xs font-semibold")
          : "relative inline-flex min-h-6 min-w-6 items-center justify-center rounded-full px-2 text-xs font-semibold text-[var(--chalk-app-text)]",
        dot && "size-3 min-h-0 min-w-0 p-0",
        className,
      )}
