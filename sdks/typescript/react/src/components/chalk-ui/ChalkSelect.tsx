...
export const ChalkSelect = forwardRef<HTMLSelectElement, ChalkSelectProps>(function ChalkSelect({ children, className, label, wrapperClassName, seed, roughness, tone = "neutral", style, ...props }, ref) {
  ...
  const control = (
    <span className={cn("group relative block min-h-10", wrapperClassName)}>
      ...
      <select
        ref={ref}
        className={cn("relative z-[1] block h-full min-h-10 w-full appearance-none rounded-md border-0 bg-transparent px-3 py-2 pr-8 text-sm text-[var(--chalk-app-text)] outline-none focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60", className)}
        style={style}
        {...props}
      >
        {children}
