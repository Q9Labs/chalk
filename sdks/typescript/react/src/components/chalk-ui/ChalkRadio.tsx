...
export const ChalkRadio = forwardRef<HTMLInputElement, ChalkRadioProps>(function ChalkRadio({ children, className, label, wrapperClassName, seed, roughness, tone = "accent", ...props }, ref) {
  ...
  return (
    <label className={cn("group inline-flex min-h-6 cursor-pointer items-center gap-2 text-sm", skin === "classic" ? "text-[var(--chalk-app-text,var(--chalk-text))]" : "text-[var(--chalk-app-text)]", wrapperClassName)}>
      <span className="relative size-5 shrink-0">
        <input
          ...
          className={cn(
            skin === "classic"
              ? "relative z-[2] size-5 cursor-pointer accent-[var(--chalk-app-control-primary,var(--chalk-accent))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--chalk-app-control-active-line,var(--chalk-focus))]"
              : "peer absolute inset-0 z-[2] size-full cursor-pointer opacity-0",
            className,
          )}
        />
