...
export const ChalkAlert = forwardRef<HTMLDivElement, ChalkAlertProps>(function ChalkAlert({ children, className, seed, roughness, style, tone = "danger", role = "alert", ...props }, ref) {
  ...
  return (
    <div ref={ref} className={cn("group relative overflow-visible rounded-lg bg-transparent px-4 py-3 text-sm text-[var(--chalk-app-text)]", className)} role={role} style={{ position: "relative", ...style }} {...props}>
