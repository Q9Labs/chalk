...
import { createSpaceClient, type ClientEventMap, type GetAccess, type JoinOptions, type SpaceClient } from "@q9labsai/chalk-client";
...
import { toAudioParticipants, toListParticipants, toParticipantNames, toVideoParticipants } from "../../selectors/space-selectors";
...
function SpaceSurface(props: ChalkProps & { readonly spaceName: string; readonly reconnecting: boolean }): React.JSX.Element {
  const client = useSpaceClient();
  const self = useSelf();
  ...
  const [settings, setSettings] = useState<SettingsDialogValue>(() => createSettings(self.displayName ?? "", props.layout ?? "focus"));
  ...
  return (
    <SpaceView
        ...
        feature("settings")
          ? {
              ...
              onUpdateAppearance: (updates) => {
                setSettings((current) => ({ ...current, appearance: { ...current.appearance, ...updates } }));
      ...
      overlay={
        commandError ? (
          <p role="alert" className="absolute right-4 bottom-24 z-50 rounded bg-[var(--chalk-danger-surface)] p-3 text-sm text-[var(--chalk-danger)]">
            {commandError}
          </p>
        ) : undefined
      }
...
function createSettings(displayName: string, layout: SpaceLayout): SettingsDialogValue {
  return {
    ...
    video: { quality: "auto" },
    appearance: { layout, theme: "system", gradient: "default", showFilmstrip: true, reducedMotion: false, generatedAvatars: true, profileGradient: { mode: "auto" }, ambientBackground: true },
