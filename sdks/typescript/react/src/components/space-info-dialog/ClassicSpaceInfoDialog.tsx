...
export const ClassicSpaceInfoDialog = React.memo<SpaceInfoDialogProps>(
  ({ isOpen, onClose, spaceName, spaceId, inviteLink, onCopyLink, isRecording = false, isTranscribing = false, duration = 0, stats = { latency: 28, packetLoss: 0.1, resolution: "1080p · 60fps", region: "Frankfurt, DE" }, className }) => {
    const skin = useSkin();
    ...
    return (
      <div data-chalk-skin={skin} className="fixed inset-0 z-50 grid place-items-center bg-[var(--chalk-app-canvas)]/90 p-4 backdrop-blur-[1px]" onMouseDown={onClose}>
        <section
        ...
        >
          <header className="flex items-start justify-between gap-5 border-b border-[var(--chalk-app-line)] px-6 py-5">
            <div className="min-w-0">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-[var(--chalk-app-text-muted)]">
                ...
                {isRecording ? <span className="rounded-full bg-[var(--chalk-app-danger)]/10 px-2 py-0.5 text-[var(--chalk-app-danger)]">Recording</span> : null}
                {isTranscribing ? <span className="rounded-full bg-[var(--chalk-app-control-active)] px-2 py-0.5 text-[var(--chalk-app-control-active-text)]">Transcribing</span> : null}
              </div>
            </div>
            <button type="button" onClick={onClose} className="grid h-9 w-9 shrink-0 place-items-center rounded-[8px] text-[var(--chalk-app-text-muted)] transition hover:bg-[var(--chalk-app-control-hover)] hover:text-[var(--chalk-app-text)]" aria-label="Close space details">
          ...
          <div className="space-y-5 p-6">
            <div>
              ...
              <div className="flex items-center gap-2 rounded-[10px] border border-[var(--chalk-app-line)] bg-[var(--chalk-app-input)] p-2">
                ...
                <button
                  ...
                  onClick={handleCopy}
                  className={cn(
                    "flex h-9 shrink-0 items-center gap-2 rounded-[7px] px-3 text-xs font-semibold transition",
                    copied ? "bg-[var(--chalk-app-control-active)] text-[var(--chalk-app-control-active-text)]" : "bg-[var(--chalk-app-control-primary)] !text-white hover:bg-[var(--chalk-app-control-primary-hover)]",
                  )}
            ...
            <div className="grid overflow-hidden rounded-[10px] border border-[var(--chalk-app-line)] bg-[var(--chalk-app-panel)] sm:grid-cols-2">
