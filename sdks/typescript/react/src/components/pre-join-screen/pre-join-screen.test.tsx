...
describe("PreJoinScreen", () => {
  it("passes the chosen identity and media preferences to the consumer", () => {
    const onJoin = vi.fn();
    render(<PreJoinScreen defaultDisplayName="Ada" initialMicrophoneEnabled initialCameraEnabled onJoin={onJoin} />);
  ...
  it("does not offer a blank identity to the session creator", () => {
    render(<PreJoinScreen defaultDisplayName="" onJoin={vi.fn()} />);
  ...
  it("uses a consumer-provided logo without assuming an application asset path", () => {
    const { rerender } = render(<PreJoinScreen defaultDisplayName="Ada" onJoin={vi.fn()} />);
    expect(screen.queryByRole("img", { name: "Chalk" })).not.toBeInTheDocument();
    expect(screen.getByText("Chalk")).toBeInTheDocument();

    rerender(<PreJoinScreen logoUrl="/chalk.svg" defaultDisplayName="Ada" onJoin={vi.fn()} />);
  ...
  it("uses the light editorial lobby without duplicating device status", () => {
    const { container } = render(<PreJoinScreen roomName="Design review" defaultDisplayName="Ada" initialMicrophoneEnabled initialCameraEnabled onJoin={vi.fn()} />);
