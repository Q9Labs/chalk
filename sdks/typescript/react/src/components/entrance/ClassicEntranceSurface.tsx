 ...
 */
export function ClassicEntranceSurface({ spaceName, logoUrl, displayName, microphone, camera, joining, error, theme, previewError, previewStream = null, onDisplayNameChange, onMicrophoneChange, onCameraChange, onSubmit, onCancel }: EntranceSurfaceProps): React.JSX.Element {
      ...
      <section className="grid w-full max-w-5xl overflow-hidden rounded-lg border border-[var(--chalk-line)] bg-[var(--chalk-surface)] shadow-[var(--chalk-shadow)] lg:grid-cols-[minmax(0,1fr)_24rem]">
        <div className="border-b border-[var(--chalk-line)] p-5 lg:border-r lg:border-b-0">
          ...
          <div className="relative aspect-video overflow-hidden rounded-md border border-[var(--chalk-line)] bg-[var(--chalk-stage)]">
            ...
            <span className="absolute bottom-3 left-3 rounded bg-[var(--chalk-app-text,var(--chalk-text))] px-2 py-1 text-xs text-[var(--chalk-app-canvas,var(--chalk-accent-text))]">{displayName.trim() || "You"}</span>
          </div>
