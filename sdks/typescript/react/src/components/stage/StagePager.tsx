...
export function StagePager({ page, pageCount, onPageChange, arrowsCenterY, dotsHeight, interactive = true }: StagePagerProps): React.JSX.Element {
  ...
  return (
    <nav className="pointer-events-none absolute inset-0 z-30" aria-label="Stage pages">
      <button type="button" className={cn(ARROW_CLASS, "absolute left-3", arrowsCenterY === null && "top-1/2 -translate-y-1/2")} style={arrowStyle} onClick={() => onPageChange(Math.max(0, page - 1))} disabled={!interactive || page === 0} aria-label="Previous page">
        <ArrowLeft01Icon size={18} />
      </button>
      <button type="button" className={cn(ARROW_CLASS, "absolute right-3", arrowsCenterY === null && "top-1/2 -translate-y-1/2")} style={arrowStyle} onClick={() => onPageChange(Math.min(pageCount - 1, page + 1))} disabled={!interactive || page >= pageCount - 1} aria-label="Next page">
      ...
      <div className="pointer-events-auto absolute inset-x-0 bottom-0 flex items-center justify-center gap-2" style={{ height: dotsHeight }} role="group" aria-label={`Page ${page + 1} of ${pageCount}`}>
        {Array.from({ length: pageCount }, (_, index) => (
          <button key={index} type="button" className="grid h-4 w-4 place-items-center rounded-full" onClick={() => onPageChange(index)} disabled={!interactive} aria-label={`Go to page ${index + 1}`} aria-current={index === page ? "page" : undefined}>
