...
import { canExecuteNativeJoin, canStartNativeJoin, shouldFailNativeJoinAfterDisconnect, shouldPromoteAfterJoinError } from "./native-join-guard";
...
describe("shouldFailNativeJoinAfterDisconnect", () => {
  it("fails a pending join once the joined room drops to disconnected before promotion", () => {
    expect(
      shouldFailNativeJoinAfterDisconnect({
        ...
        joinAttemptAgeMs: 3_100,
      }),
    ).toBe(true);
  });

  it("ignores healthy or unrelated room states", () => {
    expect(
      shouldFailNativeJoinAfterDisconnect({
        phase: "joining",
        hasPendingJoinRequest: true,
  ...
  it("waits through the websocket settle window while signaling is still connecting", () => {
    expect(
      shouldFailNativeJoinAfterDisconnect({
        phase: "joining",
        hasPendingJoinRequest: true,
  ...
  it("still fails a join that never settles after the hard timeout", () => {
    expect(
      shouldFailNativeJoinAfterDisconnect({
        phase: "joining",
        hasPendingJoinRequest: true,
