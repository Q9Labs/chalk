...
import { buildCompactParticipantPages, normalizeStageParticipants, resolveNativeSpaceLayout } from "./native-space-layout";
...
function participant(overrides: Partial<SpaceParticipant> = {}): SpaceParticipant {
  return {
...
describe("native-space-layout", () => {
  ...
  it("stays in grid mode without whiteboard or a live screen share track", () => {
    ...
    const result = resolveNativeSpaceLayout({
      participants: [local],
