...
export function AppearanceSettings(): React.JSX.Element {
  ...
  return (
    <View style={styles.section}>
      <Text style={styles.intro}>Choose a complete color family, then layer in a material texture. Every combination is available on mobile.</Text>
      {(["light", "dark"] as const).map((mode) => (
        <PaletteGroup key={mode} mode={mode} selectedPalette={appearance.palette} onSelect={setPalette} />
      ))}
      <View style={styles.textureSection}>
        ...
        <View style={styles.textureGrid}>
          {THEME_TEXTURES.map((texture) => {
            ...
            return (
              <Pressable accessibilityLabel={`Use ${texture.label} texture`} accessibilityRole="button" accessibilityState={{ selected }} key={texture.value} onPress={() => setTexture(texture.value)} style={({ pressed }) => [styles.texture, selected && styles.selected, pressed && styles.pressed]}>
                <TexturePreview kind={texture.value} background={appearance.tokens.stage} foreground={appearance.tokens.textMuted} />
                <Text numberOfLines={1} style={styles.textureLabel}>
                  {texture.label}
                </Text>
              </Pressable>
            );
...
function PaletteGroup({ mode, selectedPalette, onSelect }: { readonly mode: ThemeMode; readonly selectedPalette: string; readonly onSelect: (palette: (typeof THEME_PALETTES)[number]["value"]) => void }): React.JSX.Element {
  return (
    <View style={styles.group}>
      ...
      <View style={styles.paletteGrid}>
        {THEME_PALETTES.filter((palette) => palette.mode === mode).map((palette) => {
          ...
          return (
            <Pressable accessibilityLabel={`Use ${palette.label} palette`} accessibilityRole="button" accessibilityState={{ selected }} key={palette.value} onPress={() => onSelect(palette.value)} style={({ pressed }) => [styles.palette, selected && styles.selected, pressed && styles.pressed]}>
