import Attachment01Icon from "@hugeicons/core-free-icons/dist/esm/Attachment01Icon";
...
export function SpaceChatSheet({ controller }: { readonly controller: SpaceController }): React.JSX.Element {
  ...
  const sendDisabled = !controller.chatDraft.trim() && controller.chatAttachments.length === 0;
      ...
      {controller.chatAttachments.length ? (
        <View style={styles.selectedAttachments}>
          {controller.chatAttachments.map((attachment) => (
            <Pressable accessibilityLabel={`Remove ${attachment.fileName}`} key={attachment.attachmentId} onPress={() => controller.removeChatAttachment(attachment.attachmentId)} style={({ pressed }) => [styles.selectedAttachment, pressed && styles.pressed]}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={Platform.OS === "ios" ? 12 : 0}>
        <View style={styles.composer}>
          {controller.pickChatAttachments ? (
            <Pressable accessibilityLabel="Attach file" accessibilityRole="button" disabled={controller.chatAttachmentsLoading} onPress={controller.pickChatAttachments} style={({ pressed }) => [styles.composerButton, pressed && styles.pressed]}>
              <HugeiconsIcon color={Theme.colors.ink} icon={Attachment01Icon} size={22} />
            </Pressable>
          ) : null}
          <TextInput accessibilityLabel="Message the Space" multiline onChangeText={controller.setChatDraft} onSubmitEditing={controller.sendChatMessage} placeholder="Message the Space" placeholderTextColor={Theme.colors.placeholder} style={styles.input} value={controller.chatDraft} />
          <Pressable accessibilityLabel="Send message" accessibilityRole="button" disabled={sendDisabled} onPress={controller.sendChatMessage} style={({ pressed }) => [styles.sendButton, sendDisabled && styles.sendDisabled, pressed && styles.pressed]}>
            <HugeiconsIcon color={Theme.colors.primaryForeground} icon={Navigation03Icon} size={21} />
          </Pressable>
        </View>
