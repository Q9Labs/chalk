...
function AppearanceThemeBridge({ children, theme }: { readonly children: ReactNode; readonly theme?: ChalkTheme }): React.JSX.Element {
  const { appearance } = useNativeAppearance();
  return (
    <NativeThemeProvider appearance={appearance} theme={theme}>
      {children}
    </NativeThemeProvider>
  );
}
...
function SpaceExperience(props: ChalkProps): React.JSX.Element {
  ...
  if (connection.status === "idle" && entrance) {
    return (
      <AppearanceLayer>
        <Entrance defaultDisplayName={props.displayName} defaults={props.defaults} error={joinError ?? undefined} logoUrl={props.logoUrl} onJoin={join} spaceName={spaceName} />
      </AppearanceLayer>
    );
  }
  ...
  if (connection.status === "idle") {
    return (
      <AppearanceLayer>
        <SpaceStatus message={joinError ?? `Entering ${spaceName}…`} onRetry={joinError ? retryAutomaticJoin : undefined} />
      </AppearanceLayer>
    );
  }
  ...
  if (connection.status === "joining") {
    if (!entrance)
      return (
        <AppearanceLayer>
          <SpaceStatus message={`Entering ${spaceName}…`} />
        </AppearanceLayer>
      );
    return (
      <AppearanceLayer>
        <Entrance defaultDisplayName={props.displayName} defaults={props.defaults} joining logoUrl={props.logoUrl} onJoin={join} spaceName={spaceName} />
      </AppearanceLayer>
    );
  }
  ...
  if (connection.status === "failed") {
    if (!hasBeenLive.current && entrance) {
      return (
        <AppearanceLayer>
          <Entrance defaultDisplayName={props.displayName} defaults={props.defaults} error={connection.lastError?.message ?? "Unable to enter this Space."} logoUrl={props.logoUrl} onJoin={join} spaceName={spaceName} />
        </AppearanceLayer>
      );
    }
    return (
      <AppearanceLayer>
        <SpaceStatus message={connection.lastError?.message ?? "This Space is unavailable."} onRetry={() => void join(defaultJoinOptions(props))} />
      </AppearanceLayer>
    );
  }

  if (connection.status === "leaving")
    return (
      <AppearanceLayer>
        <SpaceStatus message={`Leaving ${spaceName}…`} />
      </AppearanceLayer>
    );
  if (connection.status === "left")
    return (
      <AppearanceLayer>
        <SpaceStatus message="You have left this Space." onRetry={() => void join(defaultJoinOptions(props))} />
      </AppearanceLayer>
    );
...
function AppearanceLayer({ children }: { readonly children: ReactNode }): React.JSX.Element {
  const theme = useNativeTheme();
  return (
    <View style={[styles.appearanceLayer, { backgroundColor: theme.colors.background }]}>
      {children}
      <NativeTextureOverlay />
    </View>
  );
