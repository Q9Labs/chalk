 ...
 */
export function useSpaceViewController({ spaceName, inviteLink, layout: controlledLayout, onLayoutChange, features, onLeave, onEndEpisode, onDiagnosticsChange }: SpaceViewProps) {
  ...
  const capabilities = useMemo(
    () => ({
      ...
      canSetParticipantRole: self.can("assignRoles"),
  ...
  const sendChatMessage = useCallback(() => {
    ...
    if (!text) return;
    setChatDraft("");
    void run(() => chat.sendMessage(text));
  }, [chat, chatDraft, run]);
  ...
  return {
    ...
    setParticipantRole: (participantId: string, role: string) => void run(() => client.participants.assignRole(participantId, role)),
