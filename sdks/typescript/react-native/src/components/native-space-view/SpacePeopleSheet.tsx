...
export function SpacePeopleSheet({ controller }: { readonly controller: SpaceController }): React.JSX.Element {
  ...
  const openActions = (participantId: string) => {
    ...
    <View style={styles.content}>
      <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
        {controller.admissionRequests.map((request) => (
          <View key={request.requestId} style={styles.admissionRequest}>
            ...
            {controller.canManageAdmission ? (
              <View style={styles.admissionActions}>
                <Pressable accessibilityRole="button" onPress={() => controller.denyAdmission(request.requestId)} style={({ pressed }) => [styles.denyButton, pressed && styles.pressed]}>
                  <Text style={styles.denyText}>Deny</Text>
                </Pressable>
                <Pressable accessibilityRole="button" onPress={() => controller.admitParticipant(request.requestId)} style={({ pressed }) => [styles.admitButton, pressed && styles.pressed]}>
        ...
        {controller.participants.participants.map((participant) => {
          ...
          return (
            <View key={participant.id} style={styles.participantRow}>
              ...
              {canAct ? (
                <Pressable accessibilityLabel={`Actions for ${participant.displayName}`} accessibilityRole="button" onPress={() => openActions(participant.id)} style={({ pressed }) => [styles.moreButton, pressed && styles.pressed]}>
      ...
      <ParticipantActionSheet controller={controller} onClose={closeActions} participantId={actionParticipantId} />
    </View>
  );
}

function canActOnParticipant(controller: SpaceController): boolean {
  return controller.canMuteParticipants || controller.canRequestMedia || controller.canStopParticipantCamera || controller.canStopParticipantScreenShare || controller.canRemoveParticipants;
}
