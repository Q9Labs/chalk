...
import { buildParticipantActionDescriptors, type ParticipantActionDescriptor } from "./space-progressive-surface-helpers";
...
export function ParticipantActionSheet({ controller, participantId, onClose }: ParticipantActionSheetProps): React.JSX.Element {
  const participant = participantId ? controller.participants.participants.find((item) => item.id === participantId) : undefined;
  const descriptors = buildParticipantActionDescriptors({
...
function ParticipantActionRow({ descriptor, onPress }: { readonly descriptor: ParticipantActionDescriptor; readonly onPress: () => void }): React.JSX.Element {
  const icon = descriptor.kind === "mute" || descriptor.kind === "requestUnmute" ? MicOff01Icon : descriptor.kind === "requestCamera" || descriptor.kind === "stopCamera" ? Video01Icon : CancelCircleIcon;
  return (
