...
import { resolveSpaceCallKitOptions, type ResolvedSpaceCallKitOptions, type SpaceCallKitOptions } from "../callkit/resolve-space-callkit-options";
import type { SpaceLifecyclePhase } from "./space-lifecycle";

export interface SpaceCallKitPort {
  readonly isSupported: boolean;
  addListener(listener: (event: CallKitEvent) => void): { remove(): void };
  configure(options: ResolvedSpaceCallKitOptions): Promise<{ isSupported: boolean }>;
...
export interface SpaceCallKitSyncInput {
  readonly callKit?: SpaceCallKitOptions | boolean;
...
export class SpaceCallKitController {
  readonly #port: SpaceCallKitPort;
  #input: SpaceCallKitSyncInput | undefined;
  ...
  constructor(port: SpaceCallKitPort = defaultCallKit) {
    this.#port = port;
  }
  ...
  readonly start = (): void => {
    ...
    this.#subscription = this.#port.addListener((event) => {
      ...
      if (event.type === "endCallAction") {
        input.onEndCall({ closeAfterLeave: input.phase !== "live" });
        return;
      }
  ...
  readonly sync = (input: SpaceCallKitSyncInput): void => {
    this.#input = input;
    ...
    if (!callKitOptions || !this.#port.isSupported) {
      ...
      if (input.phase === "entrance" || input.phase === "left") void this.endCall();
      return;
    }
    ...
    if (input.phase === "live" && this.#activeCallId && this.#reportedConnectedCallId !== this.#activeCallId) {
  ...
  #callKitOptions(input: SpaceCallKitSyncInput): ResolvedSpaceCallKitOptions | null {
    return resolveSpaceCallKitOptions({
      callKit: input.callKit,
      hasVideo: input.hasVideo,
      spaceId: input.spaceId,
      spaceName: input.spaceName || input.spaceId,
    });
  }
}
