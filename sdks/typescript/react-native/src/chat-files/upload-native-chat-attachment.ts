...
export interface NativeChatAttachmentFile {
  readonly bytes: ArrayBuffer;
  readonly fileName: string;
  readonly mimeType: string;
}

export interface UploadNativeChatAttachmentOptions {
  readonly digestSha256?: (bytes: ArrayBuffer) => Promise<string>;
  readonly fetch?: typeof globalThis.fetch;
  readonly randomUUID?: () => string;
}

export async function uploadNativeChatAttachment(file: NativeChatAttachmentFile, chatFiles: ChatFiles, options: UploadNativeChatAttachmentOptions = {}): Promise<ChalkChatAttachment> {
