...
export interface SpaceCallKitOptions extends CallKitConfiguration {
  displayName?: string;
  enabled?: boolean;
  handle?: string;
...
export interface ResolvedSpaceCallKitOptions extends CallKitConfiguration, Pick<CallKitCallOptions, "displayName" | "handle" | "handleType" | "hasVideo"> {}

export function resolveSpaceCallKitOptions({ callKit, hasVideo, spaceId, spaceName }: { callKit?: SpaceCallKitOptions | boolean; hasVideo: boolean; spaceId: string; spaceName?: string }): ResolvedSpaceCallKitOptions | null {
  if (!callKit || (typeof callKit === "object" && callKit.enabled === false)) {
    return null;
  }

  const options = typeof callKit === "object" ? callKit : {};
  const fallbackDisplayName = spaceName?.trim() || spaceId;
  const fallbackHandle = spaceId;
