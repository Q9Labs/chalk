...
describe("createV1SyncClient", () => {
  it("uses injected lifecycle, persistence, and websocket boundaries", async () => {
    ...
    const client = createV1SyncClient({
      url: "wss://sync.test/v1/sync",
      token: async () => "token",
      pendingStore: new InMemoryV1PendingTargetStore(),
      webSocket: {
        connect(url) {
    ...
    expect(client).toBeInstanceOf(V1SyncClient);
    expect(client.getSnapshot().connection.phase).toBe("idle");

    await client.start();
    expect(connections).toEqual(["wss://sync.test/v1/sync"]);
