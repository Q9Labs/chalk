...
export const makeSpaceClientCoreLayer = (options: SpaceClientOptions, platform: SpaceClientPlatform = {}) => {
  ...
  const dependencies = { ...createDefaultConnectionDependencies({ apiBaseURL: apiBaseUrl, syncURL: syncUrl, whiteboardURL: platform.whiteboardUrl }), ...platform.dependencies } satisfies ConnectionDependencies;
  const lifecycle = makeConnectionLifecycleLayer({
    access: async (request) => parseParsedAccessGrant(await (platform.connectionAccess ? platform.connectionAccess(request) : options.getAccess({ space: options.space.trim(), reason: accessReason(request?.reason) }))),
...
export const makeSpaceClientCoreLayerFromServices = (options: SpaceClientOptions, platform: SpaceClientPlatform = {}) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const controllers = yield* makeControllerEffects({ apiBaseUrl, connection: lifecycle, store, mediaDeviceSelection: selection, featureFactories: dependencies, fetch: platform.fetch });
      return yield* Effect.acquireRelease(
        Effect.sync(() => new SpaceClientCore(lifecycle, selection, store, controllers)),
...
export class SpaceClientCore {
  ...
  constructor(connection: ConnectionLifecycleCapability, _selection: MediaDeviceSelection, store: SpaceStore, controllers: ControllerEffects) {
    ...
    this.#unsubscribeConnection = connection.subscribe(() => this.#store.updateConnection(connection.getSnapshot()));
  }
  ...
  leave(): ClientEffect<void> {
    return this.#connection.leave().pipe(Effect.mapError(normalizeClientError));
  }
  endEpisode(): ClientEffect<void> {
    return this.#connection.runCommand(({ sync }) => foreign(() => sync.endEpisode()).pipe(Effect.tap(() => this.#connection.confirmEpisodeEnded))).pipe(Effect.asVoid, Effect.mapError(normalizeClientError));
  }
  extendEpisode(minutes: number): ClientEffect<void> {
    return this.#connection.runCommand(({ sync }) => foreign(() => sync.extendEpisode(minutes))).pipe(Effect.asVoid, Effect.mapError(normalizeClientError));
  }
