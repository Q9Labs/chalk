...
export interface SyncFrameObservation {
...
export interface JourneyTelemetryHost {
...
export class TelemetryJourney {
  ...
  private sequence = 0;
  private terminalEvent: TelemetryEvent | undefined;

  constructor(
    private readonly telemetry: JourneyTelemetryHost,
    context: JourneyTelemetryContext,
  ) {
...
function httpObservationAttributes(observation: HttpRequestObservation): TelemetryAttributes {
  ...
  if (observation.durationMs !== undefined) attributes.duration_ms = Math.round(observation.durationMs);
  if (observation.statusCode !== undefined) attributes.status_code = observation.statusCode;
  return attributes;
}
