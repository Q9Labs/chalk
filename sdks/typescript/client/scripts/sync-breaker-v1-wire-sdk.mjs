...
async function liveHarness(options = {}) {
  ...
  const store = options.store ?? new InMemoryV1PendingTargetStore();
  const sockets = [];
  const factory = {
    connect: () => {
  ...
  if (!socket) throw new Error("v1 replica did not create a deterministic socket");
  socket.open();
  await settle();
  ...
  await settle();
  if (client.getSnapshot().connection.phase !== "live") throw new Error(`v1 replica did not reach live after four-stream recovery (${client.getSnapshot().connection.phase})`);
...
async function nextState(state, patch) {
  ...
  return { ...next, stateDigest: await computeV1StateDigest(next) };
}
