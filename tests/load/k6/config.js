...
export const ACTIVE_USERS = parseNumberEnv(
	__ENV.K6_ACTIVE_USERS || __ENV.ACTIVE_USERS,
	3000,
);
export const ROOM_SIZE = parseNumberEnv(__ENV.K6_ROOM_SIZE, 150);
export const SHORT_RUN = parseBooleanEnv(__ENV.K6_SHORT, false);
export const EXPECT_RATE_LIMIT = parseBooleanEnv(__ENV.K6_EXPECT_RATE_LIMIT, false);
...
if (__ENV.DEBUG_ENV === "true") {
	console.log(
		`DEBUG_ENV BASE_URL=${BASE_URL} WS_URL=${WS_URL} TENANT_ID=${testTenant.id} API_KEY=${testTenant.apiKey} ACTIVE_USERS=${ACTIVE_USERS} ROOM_SIZE=${ROOM_SIZE} SHORT=${SHORT_RUN} EXPECT_RATE_LIMIT=${EXPECT_RATE_LIMIT}`,
	);
}
