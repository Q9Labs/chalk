...
const episode = await createEpisode(client, tenant.id, space.id, "webhook");
...
function createEpisode(client, tenantId, spaceId, source) {
  return Effect.runPromise(
    client.episodes.createEpisode({
      params: { tenant_id: tenantId, space_id: spaceId },
      headers: { "Idempotency-Key": idempotencyKey("episode") },
      payload: { started_at: new Date().toISOString(), metadata: { source } },
    }),
  );
}
