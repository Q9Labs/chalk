...
locals {
  ...
  plain_env = {
    ...
    OPS_WHATSAPP_ENABLED               = tostring(var.ops_whatsapp_enabled)
    OPS_TWILIO_WHATSAPP_FROM           = var.ops_twilio_whatsapp_from
    OPS_WHATSAPP_TO_CRITICAL           = var.ops_whatsapp_to_critical
    OPS_WHATSAPP_TO_MAJOR              = var.ops_whatsapp_to_major
    OPS_PUBLIC_STATUS_BASE_URL         = var.ops_public_status_base_url
  }
...
resource "aws_ssm_parameter" "ops_ingest_token" {
  count = trimspace(nonsensitive(var.ops_ingest_token)) != "" ? 1 : 0

  name  = "${trimsuffix(var.ssm_parameter_path, "/")}/OPS_INGEST_TOKEN"
  type  = "SecureString"
  value = var.ops_ingest_token
}

resource "aws_ssm_parameter" "ops_twilio_account_sid" {
  count = trimspace(nonsensitive(var.ops_twilio_account_sid)) != "" ? 1 : 0

  name  = "${trimsuffix(var.ssm_parameter_path, "/")}/OPS_TWILIO_ACCOUNT_SID"
  type  = "SecureString"
  value = var.ops_twilio_account_sid
}

resource "aws_ssm_parameter" "ops_twilio_auth_token" {
  count = trimspace(nonsensitive(var.ops_twilio_auth_token)) != "" ? 1 : 0

  name  = "${trimsuffix(var.ssm_parameter_path, "/")}/OPS_TWILIO_AUTH_TOKEN"
  type  = "SecureString"
  value = var.ops_twilio_auth_token
}
