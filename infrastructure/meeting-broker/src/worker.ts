...
export async function handleBrokerRequest(request: Request, env: WorkerEnv, log: Log = structuredLog): Promise<Response> {
  ...
  try {
    response = await route(request, env, url, requestId, log);
  } catch (error) {
...
async function route(request: Request, env: WorkerEnv, url: URL, requestId: string, log: Log): Promise<Response> {
  ...
  if (url.pathname === `${brokerPath}/browser-session`) {
    ...
    if (legacyCreationFrozen(env) && !resume) return rejectLegacyCreation(url.pathname, requestId, log);
  ...
  if (url.pathname === `${brokerPath}/client-session`) {
    ...
    if (legacyCreationFrozen(env) && input.clientSessionId === undefined) return rejectLegacyCreation(url.pathname, requestId, log);
    await enforceRateLimit(env.CREATE_RATE_LIMITER, await anonymousRateKey(request));
    const inviteToken = input.inviteToken ?? randomCapability();
...
function legacyCreationFrozen(env: WorkerEnv): boolean {
  const configured = env.CHALK_LEGACY_CREATION_FREEZE?.trim().toLowerCase();
  return configured === "1" || configured === "true" || configured === "enabled";
}

function rejectLegacyCreation(path: string, requestId: string, log: Log): Response {
  log("creation_frozen", { method: "POST", path, requestId, status: 503, retryAfterSeconds: 60 });
  return json(503, { error: "Legacy meeting creation is paused during cutover." }, { "retry-after": "60" });
}
