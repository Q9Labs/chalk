...
export class MeetingSession extends DurableObject<WorkerEnv> {
  ...
  private async access(input: InternalAccessInput): Promise<Response> {
    ...
    if (!meeting.sessionId) {
      const session = await chalk.episodes.create(this.environment.CHALK_ROOM_ID, {}, { idempotencyKey: `meeting-session-${meeting.logId}` });
    ...
    if (client.participantGeneration === undefined) {
      ...
      const admission = await chalk.participants.admit(
        ...
        sessionId,
        {
          ...
          role: client.isHost ? "owner" : "collaborator",
        },
  ...
  private async removeGuestParticipant(meeting: MeetingRecord, client: ClientRecord, trace: TraceContext): Promise<void> {
    ...
    try {
      await this.chalk(trace).participants.remove(this.environment.CHALK_ROOM_ID, meeting.sessionId, client.participantSessionId, { participantGeneration: client.participantGeneration }, { idempotencyKey: `meeting-remove-${client.participantSessionId}-${client.participantGeneration}` });
    } catch (error) {
  ...
  private async endMeeting(meeting: MeetingRecord, trace: TraceContext, reason: string): Promise<void> {
    if (meeting.sessionId) {
      try {
        await this.chalk(trace).episodes.end(this.environment.CHALK_ROOM_ID, meeting.sessionId, { idempotencyKey: `meeting-end-${meeting.logId}` });
      } catch (error) {
...
async function issueAccess(chalk: ChalkServerClient, roomId: string, sessionId: string, client: ClientRecord, input: InternalAccessInput): Promise<AccessGrant> {
  ...
  if (input.replaceMediaConnection) {
    return chalk.participants.issueAccess(roomId, sessionId, client.participantSessionId, { participantGeneration: client.participantGeneration, replaceMediaConnection: true });
  }
  if (input.currentMediaToken) {
    return chalk.participants.issueAccess(roomId, sessionId, client.participantSessionId, {
      participantGeneration: client.participantGeneration,
