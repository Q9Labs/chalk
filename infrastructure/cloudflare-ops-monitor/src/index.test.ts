...
describe("chalk ops monitor worker", () => {
  ...
  it("runs all default checks and ingests their results", async () => {
    ...
    const ingestCalls = fetchMock.mock.calls.filter(([target]) =>
      String(target).includes("/api/v1/ops/ingest/monitor-results"),
    )
  ...
  it("supports manual fetch-triggered runs", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://chalk-ops-monitor.example/run", { method: "POST" }),
      createEnv(),
    )
