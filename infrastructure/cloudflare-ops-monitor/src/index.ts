...
async function mapWithConcurrency<T, TResult>(items: readonly T[], concurrency: number, mapper: (item: T) => Promise<TResult>): Promise<TResult[]> {
...
function isValidCriticalState(value: unknown): value is CriticalIngestState {
  ...
  return typeof maybe.streak === "number" && typeof maybe.last_alerted_streak === "number" && typeof maybe.updated_at === "string";
}
...
function hasTwilioConfig(env: Env): boolean {
  return Boolean(env.OPS_TWILIO_ACCOUNT_SID && env.OPS_TWILIO_AUTH_TOKEN && env.OPS_TWILIO_WHATSAPP_FROM && env.OPS_WHATSAPP_TO_CRITICAL);
}
...
async function sendTwilioFallbackAlert(env: Env, config: RuntimeConfig, failedCriticalIngests: IngestFailure[], runID: string, impairmentStreak: number, deadlineAt: number): Promise<boolean> {
...
async function maybeSendCriticalIngestAlert(env: Env, config: RuntimeConfig, ingestFailures: IngestFailure[], runID: string, deadlineAt: number): Promise<TwilioAlertSummary> {
  ...
  const nextState: CriticalIngestState = {
    streak: nextStreak,
    last_alerted_streak: criticalImpairment ? (alertSent ? config.twilioAlertThreshold : previousState.last_alerted_streak) : 0,
