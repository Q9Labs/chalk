...
function assertFixtureCoverage(contract) {
  ...
  const ids = ["Tenant", "User", "Space", "TenantId", "UserId", "SpaceId", "ParticipantId", "UserRole", "ProviderConfig", "OpaqueJson", "SyncHelloFrame", "SyncWelcome", "SyncCommand", "SyncAck", "SyncEvent", "SyncProtocolError"];
  ...
  const description = spaceFields.find((field) => field.name === "description");
  ...
  requireCondition(JSON.stringify(user).includes("chalk.User") && JSON.stringify(space).includes("chalk.OpaqueJson"), "User or Space reference coverage is incomplete");
  ...
  ["TenantId", "UserId", "SpaceId", "ParticipantId"].forEach((id) => requireCondition(byId.get(`chalk.${id}`)?.kind === "scalar", `Brand ${id} is missing`));
  ...
  const create = operations.find((operation) => operation.id === "spaces.create");
  const get = operations.find((operation) => operation.id === "tenants.get");
  const list = operations.find((operation) => operation.id === "spaces.list");
  ...
  requireCondition(closeCodes.map((closeCode) => closeCode.code).join(",") === "1002,1011,1012,1008,1003" && sync.connection?.helloTimeoutMs === 10000 && sync.connection?.reconnectOnCloseCodes?.[0] === 1012, "Sync close and connection behavior is incomplete");
}
...
async function assertExactPins() {
  const packageJson = JSON.parse(await readFile(resolve(repositoryRoot, "tools/contract-fixture-proof/package.json"), "utf8"));
...
async function requiredPackageFootprint(names) {
  ...
  await Promise.all(names.map((name) => visit(resolve(repositoryRoot, "tools/contract-fixture-proof"), name)));
