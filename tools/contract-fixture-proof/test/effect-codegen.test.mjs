...
const schemasEmitter = resolve(repositoryRoot, "tools/contract-fixture-proof/src/emitters/effect-schemas.mjs");
const httpApiEmitter = resolve(repositoryRoot, "tools/contract-fixture-proof/src/emitters/effect-http-api.mjs");
...
describe("Effect HTTP API generation", () => {
  ...
  it("exposes required Idempotency-Key headers on all lifecycle operations", async () => {
    ...
    for (const operationName of ["CreateEpisode", "EndEpisode", "AdmitEpisodeParticipant", "RemoveEpisodeParticipant"]) {
