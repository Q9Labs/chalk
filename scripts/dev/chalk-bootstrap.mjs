...
export async function bootstrapLocalSpace({ apiOrigin, systemToken, runtimeId, fixtureMarker, fetchImpl = fetch, now = () => new Date(), fresh = false } = {}) {
  ...
  const requestSuffix = runtimeId.replace(/[^a-zA-Z0-9_-]/g, "");
  ...
  const key = existing
    ? await request(fetchImpl, apiOrigin, `/v1/tenants/${tenant.id}/api-keys/${existing.id}/rotate`, { systemToken, method: "POST", headers: { "Idempotency-Key": `chalk-local-api-key-rotate-${requestSuffix}` }, body: { expires_at: expiresAt } })
    : await request(fetchImpl, apiOrigin, `/v1/tenants/${tenant.id}/api-keys`, { systemToken, method: "POST", headers: { "Idempotency-Key": `chalk-local-api-key-${requestSuffix}` }, body: { name: keyName, scopes: brokerScopes, expires_at: expiresAt } });
