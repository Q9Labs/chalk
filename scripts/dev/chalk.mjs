...
export function createChalkSupervisor(inputConfig, { adapters = {}, output = console.log, serviceSpecs, hooks = {} } = {}) {
  ...
  function childEnvironment(serviceId) {
    ...
    const telemetryEnvironment = config.observabilityEnabled === false ? {} : { CHALK_API_OTLP_ENDPOINT: "http://127.0.0.1:4318", CHALK_API_OTLP_INSECURE: "true" };
    if (serviceId === "api") {
    ...
    if (serviceId === "sync") {
      ...
      return {
        ...
        ...(config.observabilityEnabled === false ? {} : { CHALK_SYNC_OTLP_ENDPOINT: "http://127.0.0.1:4318" }),
      };
    }
    if (serviceId === "broker")
      return {
    ...
    if (serviceId === "web") return { ...base, VITE_API_URL: config.urls.api, VITE_CHALK_DEV_SYNC_URL: base.CHALK_SYNC_URL, VITE_CHALK_DEV_MEDIA_PLANE: "cf_sfu" };
...
function formatReadySummary(config, bindings) {
  return [
    ...
    config.observabilityEnabled === false ? "Observability disabled" : "Grafana   http://127.0.0.1:3000/d/chalk-observability-v1/chalk-observability",
...
export function discoverBrokerRuntime(root) {
  const candidates = findFiles(join(root, "infrastructure")).filter(({ path, content }) => basename(path) === "wrangler.toml" && /\/local-chalk\/\*/.test(content) && parseRequiredBindings(content).includes("CHALK_SPACE_ID"));
