  ...
  return (
    <ChalkProvider apiUrl={CHALK_API_URL} tokenProvider={tokenProvider} debug>
      <SafeAreaView style={[styles.container, isDarkMode && styles.dark]}>
        ...
        {screen === "home" && <HomeScreen onJoin={(id) => navigateTo("precall", id)} />}
        {screen === "precall" && <PreCallScreen roomId={roomId} onJoin={() => navigateTo("call")} onBack={() => navigateTo("home")} />}
        {screen === "call" && <CallScreen roomId={roomId} onLeave={() => navigateTo("home")} />}
      </SafeAreaView>
    </ChalkProvider>
  );
}
