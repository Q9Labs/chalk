...
import { APIClient, WSClient, type ConferenceClientConfig, type JoinSessionResponse, type JoinSessionConfig, type SessionConnectionState } from "@q9labs/chalk-core";
...
export function ChalkProvider({ children, apiKey, token, tokenProvider, apiUrl, wsUrl, debug, demoMode }: ChalkProviderProps) {
  ...
  const [connectionState, setConnectionStatus] = useState<SessionConnectionState>("disconnected");
  ...
  }, [apiKey, token, tokenProvider, resolvedApiUrl, resolvedWsUrl, debug, demoMode]);
  ...
  useEffect(() => {
    ...
    const lastPongAgeMs = Math.max(0, Date.now() - (wsClient.lastPongReceived ?? Date.now()));
    logger.info({
  ...
  const joinSession = useCallback(
    async (roomId: string, config: JoinSessionConfig): Promise<JoinSessionResponse> => {
      ...
      try {
        ...
        const response = demoMode ? await apiClient.demoJoin(roomId, config.displayName) : await apiClient.addParticipant(roomId, config.displayName, undefined, config.metadata);
    ...
    [apiClient, rtcManager, demoMode, initRtk, wsClient, resolvedWsUrl],
  );
  ...
  const value = useMemo<ChalkContextValue>(
    ...
    [apiClient, wsClient, wsConnectionState, wsRoomId, wsParticipantId, rtkClient, rtcManager, connectionState, roomInfo, joinSession, leaveRoom, createSession],
  );
