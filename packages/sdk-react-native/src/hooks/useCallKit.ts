...
interface CallKitModuleType {
  reportIncomingCall: (uuid: string, handle: string, displayName: string, hasVideo: boolean) => Promise<void>;
  reportOutgoingCall: (uuid: string, handle: string) => Promise<void>;
  reportCallConnected: (uuid: string) => Promise<void>;
  reportCallEnded: (uuid: string, reason: "failed" | "remoteEnded" | "unanswered" | "declinedElsewhere") => Promise<void>;
...
const CallKitModule = Platform.OS === "ios" ? (NativeModules.CallKitModule as CallKitModuleType | undefined) : undefined;
...
export interface UseCallKitResult {
  ...
  reportCallEnded: (callId: string, reason?: "failed" | "remoteEnded" | "unanswered" | "declinedElsewhere") => Promise<void>;
...
export function useCallKit(): UseCallKitResult {
  ...
  const reportIncomingCall = useCallback(async (callId: string, callerName: string, hasVideo = true) => {
    if (!CallKitModule) {
      return;
    }

    try {
      await CallKitModule.reportIncomingCall(callId, callId, callerName, hasVideo);
    } catch (err) {
      throw err;
    }
  }, []);
  ...
  const reportCallEnded = useCallback(
    async (callId: string, reason: "failed" | "remoteEnded" | "unanswered" | "declinedElsewhere" = "remoteEnded") => {
      if (!CallKitModule) {
        return;
      }
