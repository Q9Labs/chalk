...
function toChalkParticipantFromRtk(rtkParticipant: RealtimeKitParticipant, isLocal = false): Participant {
  return {
...
export function useParticipants(): UseParticipantsResult {
  const { wsClient, wsConnectionState, wsParticipantId, rtkClient, roomInfo } = useChalk();
  ...
  useEffect(() => {
    ...
    const unsubscribeUpdated = wsClient.on("participant.updated", ({ participantId, changes }) => {
      setParticipants((prev) => {
        const next = prev.map((p) => (p.id === participantId ? { ...p, ...changes } : p));
        return next;
      });
    });
  ...
  useEffect(() => {
    ...
    const remotes = rtkParticipants.map((p: RealtimeKitParticipant) => toChalkParticipantFromRtk(p, false));
    ...
    const all = localParticipantFallback ? [localParticipantFallback, ...remotes] : remotes;
    setParticipants(all);
  }, [wsClient, wsConnectionState, rtkClient, localId, roomInfo?.room?.name, roomInfo?.role]);
  ...
  const enrichedParticipants = useMemo(() => {
    ...
    const rtkParticipants = rtkClient.participants.toArray?.() ?? [];
    const rtkMap = new Map(rtkParticipants.map((p: RealtimeKitParticipant) => [p.id, p]));
    return participants.map((p) => {
      const rtk = (localId && p.id === localId ? rtkClient.self : undefined) ?? rtkMap.get(p.id);
      if (!rtk) return p;
      const base = wsClient
        ? p
        : {
  ...
  const localParticipant = useMemo(() => enrichedParticipants.find((p) => p.id === localId) ?? null, [enrichedParticipants, localId]);

  const remoteParticipants = useMemo(() => enrichedParticipants.filter((p) => !p.isLocal), [enrichedParticipants]);

  const getParticipant = useCallback((id: string) => enrichedParticipants.find((p) => p.id === id), [enrichedParticipants]);
