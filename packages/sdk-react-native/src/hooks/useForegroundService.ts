...
const CallServiceModule = Platform.OS === "android" ? (NativeModules.CallServiceModule as CallServiceModuleType | undefined) : undefined;
...
export function useForegroundService(): UseForegroundServiceResult {
  ...
  const isAvailable = Platform.OS === "android" && CallServiceModule !== undefined;
  ...
  useEffect(() => {
    ...
    // Check initial state
    CallServiceModule.isServiceRunning()
      .then(setIsRunning)
      .catch(() => {
        setIsRunning(false);
      });
  ...
  const startService = useCallback(async (roomId: string, roomName: string) => {
