...
export function useInteractions(): UseInteractionsReturn {
	...
	const reactionTimeouts = useRef<Map<string, ReturnType<typeof setTimeout>>>(
		new Map(),
	);
	...
	const addReaction = useCallback(
		(reaction: Reaction) => {
			...
			const active: ActiveReaction = {
				...
				timestamp:
					reaction.timestamp instanceof Date
	...
	useEffect(() => {
		...
		const unsubscribeParticipantUpdated = wsClient.on(
			"participant.updated",
		...
		const unsubscribeParticipantLeft = wsClient.on(
			"participant.left",
			({ participantId }) => {
				setRaisedHands((prev) => prev.filter((id) => id !== participantId));
			},
