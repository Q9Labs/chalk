...
mock.module("react-native", () => ({
  NativeModules: {
    AudioSessionModule: {
      ...
      getAvailableRoutes: mock(() => Promise.resolve(["speaker", "earpiece", "bluetooth"])),
...
describe("useScreenShare", () => {
  ...
  it("exports UseScreenShareResult interface with correct shape", () => {
    const expectedKeys = ["isScreenSharing", "startScreenShare", "stopScreenShare", "error"];
...
describe("useAudioRouting", () => {
  it("exports AudioRoute type with valid values", () => {
    const validRoutes: Array<"speaker" | "earpiece" | "bluetooth" | "headphones"> = ["speaker", "earpiece", "bluetooth", "headphones"];
  ...
  it("exports UseAudioRoutingResult interface with correct shape", () => {
    const expectedKeys = ["currentRoute", "availableRoutes", "setRoute", "isSpeakerOn", "toggleSpeaker"];
...
describe("@q9labs/chalk-react-native hooks exports", () => {
  it("exports all expected hook types", () => {
    const expectedHooks = ["useAudioRouting", "useScreenShare", "useChat", "useDevices", "useMedia", "useParticipants", "useRecording", "useRoom"];
    expect(expectedHooks.length).toBe(8);
  });

  it("exports type definitions for new hooks", () => {
    const expectedTypes = ["UseAudioRoutingResult", "UseScreenShareResult", "AudioRoute"];
