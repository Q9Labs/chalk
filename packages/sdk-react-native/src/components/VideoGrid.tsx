...
export function VideoGrid({ participants, style, gap = CHALK_THEME.spacing.sm }: VideoGridProps) {
  ...
  const layout = useMemo(() => getGridLayout(participants.length, width, height), [participants.length, width, height]);
  ...
  return (
    <View style={[styles.container, style]}>
      {rows.map((row, rowIndex) => (
        <View key={`row-${rowIndex}`} style={[styles.row, { gap, marginBottom: rowIndex < rows.length - 1 ? gap : 0 }]}>
          {row.map((participant) => (
            <ParticipantTile key={participant.id} participant={participant} style={{ width: tileWidth, height: tileHeight }} />
          ))}
          {/* Fill empty slots in last row */}
          {row.length < layout.cols && Array.from({ length: layout.cols - row.length }).map((_, i) => <View key={`empty-${i}`} style={{ width: tileWidth, height: tileHeight }} />)}
        </View>
      ))}
    </View>
  );
