...
import { createTokenProvider, type JoinSessionConfig, type TokenStorage } from "@q9labs/chalk-core";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { ActivityIndicator, StyleSheet, Text, TouchableOpacity, View, type ViewStyle } from "react-native";
import { ChalkProvider, type ChalkProviderProps, useChalk, useOptionalChalk } from "../../ChalkProvider";
...
interface VideoConferenceScreenProps extends Omit<VideoConferenceProps, "provider" | "audioSession" | "apiKey" | "token" | "tokenProvider" | "apiUrl" | "wsUrl" | "debug" | "demoMode"> {}

function ForegroundServiceController({ enabled = true, startOn = "connected", notification, requestNotificationPermission = true }: NonNullable<VideoConferenceProps["foregroundService"]>) {
  const { isAvailable, isRunning, startService, stopService, updateNotification } = useForegroundService();
  const { status, roomId, roomInfo } = useRoom();
  const { requestNotificationPermission: requestPermission } = usePermissions();

  const shouldStart = enabled && isAvailable && (startOn === "connecting" ? status === "connecting" || status === "connected" : status === "connected");
  ...
  }, [enabled, isAvailable, shouldStart, roomId, roomInfo?.name, isRunning, startService, stopService, requestPermission, requestNotificationPermission]);

  useEffect(() => {
    if (!enabled || !isAvailable || !isRunning || !notification) return;
    updateNotification(notification.title ?? "Chalk Call", notification.body ?? "Call in progress").catch(() => {});
  }, [enabled, isAvailable, isRunning, notification?.title, notification?.body, updateNotification]);
...
function MissingProvider({ style }: { style?: ViewStyle }) {
  return (
    <View style={[styles.container, styles.centerContent, style]}>
      ...
      <Text style={styles.errorMessage}>Pass the provider prop to VideoConference or wrap your tree with ChalkProvider.</Text>
    </View>
  );
}

function VideoConferenceScreen({ roomId, displayName, autoJoin = false, createIfMissing = true, roomName, joinOptions, onJoin, onEnd, onError, onLeave, endBehavior = "end", foregroundService, style }: VideoConferenceScreenProps) {
  ...
  useEffect(() => {
    if (phase === "meeting" && status === "disconnected" && hasConnectedRef.current && !isLeavingRef.current) {
      finalizeEnd();
    }
  ...
  const handleJoin = useCallback(
    ...
    [isJoining, isConnected, joinSession, resolvedRoomId, joinOptions, onJoin, onError],
  );
  ...
  useEffect(() => {
    ...
    handleJoin(name);
  }, [autoJoin, displayName, apiClient, rtcManager, resolvedRoomId, handleJoin]);
  ...
  const foregroundConfig = useMemo(
    () => ({
      ...
      requestNotificationPermission: foregroundService?.requestNotificationPermission ?? true,
    }),
    [foregroundService?.enabled, foregroundService?.startOn, foregroundService?.notification, foregroundService?.requestNotificationPermission],
  );
    ...
    return (
      <>
        <View style={[styles.container, styles.centerContent, style]}>
          ...
          <Text style={styles.errorMessage}>Provide a roomId or enable createIfMissing.</Text>
        </View>
      ...
      <>
        <View style={[styles.container, styles.centerContent, style]}>
          ...
          <View style={styles.errorActions}>
            ...
            <TouchableOpacity style={styles.cancelButton} onPress={() => onLeave?.()} activeOpacity={0.8}>
    ...
    return (
      <>
        <View style={[styles.container, styles.centerContent, style]}>
          ...
          <Text style={styles.statusText}>{isCreatingRoom ? "Creating room..." : "Preparing room..."}</Text>
        </View>
        <ForegroundServiceController {...foregroundConfig} />
      </>
    );
  ...
  if (phase === "lobby") {
    return (
      <>
        <PreJoinLobby roomId={resolvedRoomId ?? undefined} initialName={displayName} onJoin={handleJoin} error={error ?? undefined} style={style} />
        <ForegroundServiceController {...foregroundConfig} />
      </>
    );
  }
    ...
    return (
      <>
        <View style={[styles.container, styles.centerContent, style]}>
          ...
          {resolvedRoomId && <Text style={styles.statusSubtext}>{resolvedRoomId}</Text>}
        </View>
        <ForegroundServiceController {...foregroundConfig} />
      </>
    );
  ...
  if (phase === "end") {
    return (
      <>
        <EndScreen roomId={resolvedRoomId ?? roomId ?? "unknown"} duration={meetingDuration} participantCount={lastParticipantCountRef.current} onRejoin={handleRejoin} onLeave={() => onLeave?.()} />
        <ForegroundServiceController {...foregroundConfig} />
      </>
    );
  }
...
export function VideoConference(props: VideoConferenceProps) {
  const { provider, apiKey, token, tokenProvider, apiUrl, wsUrl, debug, demoMode, tokenStorage, audioSession, foregroundService, ...screenProps } = props;
  ...
  const providerProps = useMemo<ProviderConfig | undefined>(() => {
    ...
    if (!resolvedTokenProvider && apiKey && !token) {
      let storage: "sessionStorage" | "localStorage" | TokenStorage | undefined = tokenStorage;
      if (!storage) {
        try {
    ...
    if (apiKey || token || resolvedTokenProvider || apiUrl || wsUrl || debug !== undefined || demoMode !== undefined) {
      return {
        apiKey,
        token,
    ...
    return undefined;
  }, [provider, apiKey, token, tokenProvider, apiUrl, wsUrl, debug, demoMode, tokenStorage]);

  const content = hasProvider ? (
    <VideoConferenceScreen {...screenProps} foregroundService={foregroundService} />
  ) : providerProps ? (
