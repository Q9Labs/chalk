...
export function PreJoinLobby({ onJoin, roomId, initialName = "", error, style }: PreJoinLobbyProps) {
  const [displayName, setDisplayName] = useState(initialName);
  const [showCameraSelector, setShowCameraSelector] = useState(false);
  const [showMicSelector, setShowMicSelector] = useState(false);

  const { hasRequiredPermissions, requestPermissions, isChecking } = usePermissions();
  const { cameras, microphones, selectedCamera, selectedMicrophone, selectCamera, selectMicrophone } = useDevices();
  ...
  return (
    <View style={[styles.container, style]}>
      ...
      <View style={styles.previewContainer}>
        {stream ? (
          ...
          <View style={styles.avatarContainer}>
            ...
            {!hasRequiredPermissions && <Text style={styles.permissionHint}>Camera access required</Text>}
          </View>
        )}
      </View>
      ...
      <View style={styles.deviceButtons}>
        <TouchableOpacity style={styles.deviceButton} onPress={() => setShowCameraSelector(true)} disabled={cameras.length === 0}>
          <CameraIcon size={20} color={cameras.length > 0 ? CHALK_THEME.colors.primary : CHALK_THEME.colors.text.muted} />
          <Text style={[styles.deviceButtonText, cameras.length === 0 && styles.deviceButtonTextDisabled]} numberOfLines={1}>
            {cameras.length > 0 ? "Camera" : "No cameras"}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.deviceButton} onPress={() => setShowMicSelector(true)} disabled={microphones.length === 0}>
          <MicrophoneIcon size={20} color={microphones.length > 0 ? CHALK_THEME.colors.primary : CHALK_THEME.colors.text.muted} />
          <Text style={[styles.deviceButtonText, microphones.length === 0 && styles.deviceButtonTextDisabled]} numberOfLines={1}>
      ...
      {/* Name input */}
      <TextInput style={styles.nameInput} placeholder="Enter your name" placeholderTextColor={CHALK_THEME.colors.text.muted} value={displayName} onChangeText={setDisplayName} autoCapitalize="words" autoCorrect={false} returnKeyType="done" onSubmitEditing={handleJoin} />

      {/* Join button */}
      <TouchableOpacity style={[styles.joinButton, !canJoin && styles.joinButtonDisabled]} onPress={handleJoin} disabled={!canJoin} activeOpacity={0.8}>
        <Text style={[styles.joinButtonText, !canJoin && styles.joinButtonTextDisabled]}>Join</Text>
      </TouchableOpacity>
      ...
      {!hasRequiredPermissions && !isChecking && (
        <TouchableOpacity style={styles.permissionButton} onPress={requestPermissions}>
          <Text style={styles.permissionButtonText}>Grant camera & mic access</Text>
        </TouchableOpacity>
      )}

      {/* Device selectors */}
      <DeviceSelector visible={showCameraSelector} devices={cameras} selectedId={selectedCamera ?? undefined} onSelect={handleCameraSelect} onClose={() => setShowCameraSelector(false)} type="video" />

      <DeviceSelector visible={showMicSelector} devices={microphones} selectedId={selectedMicrophone ?? undefined} onSelect={handleMicSelect} onClose={() => setShowMicSelector(false)} type="audio" />
    </View>
  );
}
