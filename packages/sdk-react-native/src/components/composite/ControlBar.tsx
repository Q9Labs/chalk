...
import { MicrophoneIcon, VideoIcon, ScreenShareIcon as ScreenIcon, ChatIcon as ChatBubbleIcon, PhoneIcon } from "../../icons";
...
export function ControlBar({ onToggleAudio, onToggleVideo, onToggleScreenShare, onToggleChat, onLeave, isAudioEnabled = true, isVideoEnabled = true, isScreenSharing = false, isChatOpen = false, style }: ControlBarProps) {
  ...
  return (
    <View style={[styles.container, style]}>
      {/* Audio toggle */}
      <TouchableOpacity style={[styles.button, buttonStyle, !isAudioEnabled && styles.buttonInactive]} onPress={onToggleAudio} activeOpacity={0.7}>
        <MicrophoneIcon size={ICON_SIZE} color={iconColor} />
        {!isAudioEnabled && <View style={styles.slashOverlay} />}
      </TouchableOpacity>

      {/* Video toggle */}
      <TouchableOpacity style={[styles.button, buttonStyle, !isVideoEnabled && styles.buttonInactive]} onPress={onToggleVideo} activeOpacity={0.7}>
        <VideoIcon size={ICON_SIZE} color={iconColor} />
        {!isVideoEnabled && <View style={styles.slashOverlay} />}
      </TouchableOpacity>

      {/* Screen share toggle */}
      <TouchableOpacity style={[styles.button, buttonStyle, isScreenSharing && styles.buttonActive]} onPress={onToggleScreenShare} activeOpacity={0.7}>
        <ScreenIcon size={ICON_SIZE} color={isScreenSharing ? activeIconColor : iconColor} />
      </TouchableOpacity>

      {/* Chat toggle */}
      <TouchableOpacity style={[styles.button, buttonStyle, isChatOpen && styles.buttonActive]} onPress={onToggleChat} activeOpacity={0.7}>
        <ChatBubbleIcon size={ICON_SIZE} color={isChatOpen ? activeIconColor : iconColor} />
      </TouchableOpacity>

      {/* Leave button */}
      <TouchableOpacity style={[styles.button, buttonStyle, styles.buttonLeave]} onPress={onLeave} activeOpacity={0.7}>
