...
import { FlatList, Keyboard, KeyboardAvoidingView, Platform, StyleSheet, Text, TextInput, TouchableOpacity, View, type ViewStyle } from "react-native";
...
function MessageBubble({ message, isLocal }: { message: ChatMessage; isLocal: boolean }) {
  return (
    <View style={[styles.messageBubble, isLocal ? styles.localBubble : styles.remoteBubble]}>
      {!isLocal && <Text style={styles.senderName}>{message.senderName}</Text>}
      <Text style={[styles.messageText, isLocal && styles.localMessageText]}>{message.content}</Text>
      <Text style={[styles.timestamp, isLocal && styles.localTimestamp]}>{formatTime(message.timestamp)}</Text>
    </View>
  );
}

export function ChatPanel({ messages, onSend, localUserId, style }: ChatPanelProps) {
  ...
  return (
    <KeyboardAvoidingView style={[styles.container, style]} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 20}>
      <FlatList
        ref={flatListRef}
        data={messages}
      ...
      <View style={styles.inputContainer}>
        <TextInput style={styles.input} value={inputText} onChangeText={setInputText} placeholder="Type a message..." placeholderTextColor={CHALK_THEME.colors.text.muted} returnKeyType="send" onSubmitEditing={handleSend} blurOnSubmit={false} />
        <TouchableOpacity style={[styles.sendButton, isDisabled && styles.sendButtonDisabled]} onPress={handleSend} disabled={isDisabled} activeOpacity={0.7}>
