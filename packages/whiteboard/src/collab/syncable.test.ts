...
describe("filterSyncableElements", () => {
  it("keeps visible elements and every server-authoritative tombstone", () => {
    ...
    expect(filterSyncableElements([visible, invisible, recentTombstone, staleTombstone] as never)).toEqual([visible, recentTombstone, staleTombstone]);
  });
});
