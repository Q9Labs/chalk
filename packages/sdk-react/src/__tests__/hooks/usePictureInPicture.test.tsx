...
import {
	...
	useSharedPictureInPicture,
} from "../../components/full/picture-in-picture/PictureInPictureContext";
...
function SharedPhaseHarness({
	...
	return (
		<SharedPictureInPictureProvider enabled>
			{phase === "prejoin" ? <PreJoinRegistrar /> : <MeetingRegistrar />}
		</SharedPictureInPictureProvider>
	);
}
...
describe("usePictureInPicture", () => {
	...
	it("keeps the same Document PiP window across phase transitions", async () => {
		...
		const { getByText, rerender } = render(
			<SharedPhaseHarness phase="prejoin" />,
		);
