...
const mockCreateJoinToken = vi.fn(async () => ({ joinToken: "join-token-123" }));
...
vi.mock("../../utils/browserNavigation", () => {
  const replaceWindowLocation = vi.fn();
  (globalThis as any).__vcReplaceWindowLocationMock = replaceWindowLocation;
  return {
...
vi.mock("../../context/chalk-provider", () => ({
  useChalkSession: () => ({
    session: {
      on: vi.fn(() => () => {}),
      createJoinToken: mockCreateJoinToken,
...
describe("VideoConference pre-join devices", () => {
  ...
  beforeEach(() => {
    ...
    mockCreateJoinToken.mockClear();
    mockCreateJoinToken.mockResolvedValue({ joinToken: "join-token-123" });
    (globalThis as any).__vcReplaceWindowLocationMock?.mockClear?.();
  ...
  it("redirects mobile room flows into the app before rendering the lobby", async () => {
    Object.defineProperty(globalThis.navigator, "userAgent", {
      configurable: true,
    ...
    expect(queryByText("Ask to join")).toBeNull();
  });

  it("falls back to the mobile store when app handoff does not succeed", async () => {
    Object.defineProperty(globalThis.navigator, "userAgent", {
      configurable: true,
      value: "Mozilla/5.0 (Linux; Android 14; Pixel 8)",
    });
  ...
  it("tries iOS bundle-scheme fallback and then opens the App Store when handoff fails", async () => {
    Object.defineProperty(globalThis.navigator, "userAgent", {
      configurable: true,
      value: "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4 like Mac OS X)",
    });
    render(<VideoConference joinToken="join-token-123" userName="Hasan" />);
