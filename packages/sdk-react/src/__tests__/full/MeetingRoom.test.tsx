...
describe("MeetingRoom", () => {
  ...
  it("opens diagnostics with alt+shift+d", () => {
    const onOpenDebug = vi.fn();

    render(<MeetingRoom roomName="Test Room" localParticipant={localParticipant} participants={participants} enableTour={false} onOpenDebug={onOpenDebug} />);

    fireEvent.keyDown(document, { key: "D", altKey: true, shiftKey: true });

    expect(onOpenDebug).toHaveBeenCalledTimes(1);
    expect(vibrateSpy).toHaveBeenCalled();
  });

  it("opens diagnostics with ctrl+d", () => {
    const onOpenDebug = vi.fn();
  ...
  it("does not trigger diagnostics shortcut from editable inputs", () => {
    const onOpenDebug = vi.fn();
    const { getByRole } = render(
      <>
        <input aria-label="Outside input" />
        <MeetingRoom roomName="Test Room" localParticipant={localParticipant} participants={participants} enableTour={false} onOpenDebug={onOpenDebug} />
      </>,
    );
    ...
    fireEvent.click(getByRole("button", { name: "Diagnostics" }));
    expect(onOpenDebug).toHaveBeenCalledTimes(1);
  });

  it("opens diagnostics from the mobile more sheet", () => {
    const onOpenDebug = vi.fn();
    const originalMatchMedia = window.matchMedia;
    window.matchMedia = vi.fn().mockImplementation((query: string) => ({
      matches: query === "(max-width: 639px)",
    ...
    try {
      const { getByRole } = render(<MeetingRoom roomName="Test Room" localParticipant={localParticipant} participants={participants} enableTour={false} onOpenDebug={onOpenDebug} />);
