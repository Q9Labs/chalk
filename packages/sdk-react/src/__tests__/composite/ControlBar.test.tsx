...
describe("ControlBar", () => {
  ...
  it("renders only specified buttons", () => {
    const { getByLabelText, queryByLabelText } = render(<ControlBar buttons={["mic", "leave"]} />);
    expect(getByLabelText("Mute")).toBeDefined();
    expect(getByLabelText("Leave")).toBeDefined();
  ...
  it("hydrates dock device menus from browser enumeration when props are empty", async () => {
    ...
    const { container, getByText } = render(<ControlBar variant="dock" selectedAudioInput="mic-1" selectedAudioOutput="spk-1" selectedVideoInput="cam-1" onAudioInputChange={() => {}} onAudioOutputChange={() => {}} onVideoInputChange={() => {}} />);
