...
describe("MeetingHeader", () => {
  ...
  it("displays status badges when active", () => {
    const { getByLabelText } = render(<MeetingHeader {...defaultProps} isRecording={true} isTranscribing={true} />);
  ...
  it("calls onLayoutChange when layout button is clicked", () => {
    ...
    const { getByLabelText } = render(<MeetingHeader {...defaultProps} onLayoutChange={onLayoutChange} />);
