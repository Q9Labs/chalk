...
describe("NoiseSuppressionToggle", () => {
  it("renders correctly", () => {
    const { getByText, getByRole } = render(<NoiseSuppressionToggle enabled={false} onChange={() => {}} />);
  ...
  it("shows level selector when enabled", () => {
    const { getByRole } = render(<NoiseSuppressionToggle enabled={true} onChange={() => {}} onLevelChange={() => {}} />);
  ...
  it("reflects enabled state on the switch", () => {
    const { getByRole, rerender } = render(<NoiseSuppressionToggle enabled={false} onChange={() => {}} />);
    expect(getByRole("switch")).toHaveAttribute("aria-checked", "false");
