...
describe("DeviceSelector", () => {
  ...
  it("renders device options", () => {
    const { getByRole, getByText } = render(<DeviceSelector type="audioinput" devices={devices} onChange={() => {}} />);
  ...
  it("calls onChange when device is selected", () => {
    ...
    const { getByRole, getByText } = render(<DeviceSelector type="audioinput" devices={devices} onChange={onChange} />);
    fireEvent.click(getByRole("button", { name: "Select device" }));
  ...
  it("shows audio indicator for audio input", () => {
    const { getByRole } = render(<DeviceSelector type="audioinput" devices={devices} onChange={() => {}} />);
    expect(getByRole("status")).toBeDefined();
  });

  afterEach(() => {
    delete (
      window.HTMLMediaElement.prototype as HTMLMediaElement & {
  ...
  it("plays a routed test tone for audio output", async () => {
    ...
    const { container, getByLabelText } = render(<DeviceSelector type="audiooutput" devices={[{ deviceId: "spk-1", kind: "audiooutput", label: "Speaker 1", groupId: "g1", toJSON: () => ({}) }]} selectedDeviceId="spk-1" onChange={() => {}} />);
