...
describe("RecordingControls", () => {
  it("renders correctly in ready state", () => {
    const { getByText, getByLabelText } = render(<RecordingControls isRecording={false} />);
  ...
  it("renders correctly in recording state", () => {
    const { getByLabelText, getByText } = render(<RecordingControls isRecording={true} duration={10} />);
  ...
  it("handles start action", () => {
    const onStart = vi.fn();
    const { getByLabelText } = render(<RecordingControls isRecording={false} onStart={onStart} />);
