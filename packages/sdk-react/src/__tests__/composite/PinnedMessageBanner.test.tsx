...
describe("PinnedMessageBanner", () => {
  ...
  it("calls onUnpin when unpin button clicked", () => {
    const onUnpin = vi.fn();
    const { getByLabelText } = render(<PinnedMessageBanner message={message} onUnpin={onUnpin} />);
  ...
  it("calls onJumpToMessage when content clicked", () => {
    ...
    const { getByText } = render(<PinnedMessageBanner message={message} onJumpToMessage={onJumpToMessage} />);
