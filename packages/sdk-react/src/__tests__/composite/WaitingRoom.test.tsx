...
describe("WaitingRoom", () => {
  const participants = [{ id: "1", displayName: "Alice", joinedAt: new Date() }];

  it("renders correctly", () => {
    const { getByText } = render(<WaitingRoom participants={participants} onAdmit={() => {}} onDeny={() => {}} />);
  ...
  it("calls onAdmit when check button clicked", () => {
    const onAdmit = vi.fn();
    const { getByLabelText } = render(<WaitingRoom participants={participants} onAdmit={onAdmit} onDeny={() => {}} />);
  ...
  it("shows empty message when no one is waiting", () => {
    const { getByText } = render(<WaitingRoom participants={[]} onAdmit={() => {}} onDeny={() => {}} />);
