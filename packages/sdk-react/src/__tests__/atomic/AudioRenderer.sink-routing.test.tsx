...
const originalSetSinkIdDescriptor = Object.getOwnPropertyDescriptor(mediaPrototype, "setSinkId");
...
describe("AudioRenderer sink routing", () => {
  it("routes remote mic and screen-share audio to selected output device", async () => {
    ...
    const setSinkId = vi.fn().mockImplementation(function (this: any, sinkId: string) {
      this.sinkId = sinkId;
    ...
    const { rerender } = render(<AudioRenderer participants={[participant]} audioOutputDeviceId="spk-1" />);
    ...
    rerender(<AudioRenderer participants={[participant]} audioOutputDeviceId="spk-2" />);
