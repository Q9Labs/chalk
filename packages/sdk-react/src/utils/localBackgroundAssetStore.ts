...
const supportsIndexedDb = () => typeof window !== "undefined" && typeof indexedDB !== "undefined";
...
export const persistLocalBackgroundAsset = async (file: Blob) => {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("Failed to persist background asset"));
  });
...
export const loadLocalBackgroundAsset = async (assetKey: string) => {
  ...
  const result = await new Promise<Blob | null>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("Failed to load background asset"));
  });
