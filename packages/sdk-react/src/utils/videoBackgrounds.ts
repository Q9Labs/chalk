...
export type VideoBackgroundPresetId =
	(typeof VIDEO_BACKGROUND_PRESETS)[number]["id"];

export type StoredVideoBackgroundEffect =
	| { type: "none" }
	| { type: "blur"; blurStrength?: number }
	| { type: "preset"; presetId: VideoBackgroundPresetId }
	| { type: "custom"; assetKey: string; fileName?: string };

export const DEFAULT_STORED_VIDEO_BACKGROUND_EFFECT: StoredVideoBackgroundEffect = {
	type: "none",
};

export const getStoredVideoBackgroundEffectId = (
	effect: StoredVideoBackgroundEffect | undefined,
): string => {
...
export const getVideoBackgroundPreset = (presetId: string) =>
	VIDEO_BACKGROUND_PRESETS.find((preset) => preset.id === presetId);

export const toRuntimeVideoBackgroundEffect = (
	effect: StoredVideoBackgroundEffect | undefined,
	...
	if (effect.type === "preset") {
		return {
			mode: "image",
			imageUrl:
				getVideoBackgroundPreset(effect.presetId)?.imageUrl ?? "",
		};
	}
...
export const areVideoBackgroundEffectsEqual = (
	left: VideoBackgroundEffect | undefined,
	right: VideoBackgroundEffect | undefined,
) => {
	if (!left || !right) {
		return left === right;
	}
