...
export function useWhiteboard(): UseWhiteboardReturn {
  ...
  const sendUpdate = useCallback((elements: unknown[], files?: Record<string, unknown>, _seq?: number): void => whiteboard.sendUpdate(elements, files, _seq), [whiteboard]);

  const sendCursor = useCallback((x: number, y: number): void => whiteboard.sendCursor(x, y), [whiteboard]);

  const requestSync = useCallback((): void => whiteboard.requestSync(), [whiteboard]);

  const clear = useCallback((): void => whiteboard.clear(), [whiteboard]);

  const grantPermission = useCallback((participantId: string): void => whiteboard.grantPermission(participantId), [whiteboard]);

  const revokePermission = useCallback((participantId: string): void => whiteboard.revokePermission(participantId), [whiteboard]);
