...
export function useInteractions(): UseInteractionsReturn {
  ...
  const raiseHand = useCallback((): void => interactions.raiseHand(), [interactions]);

  const lowerHand = useCallback((): void => interactions.lowerHand(), [interactions]);

  const toggleHand = useCallback((): void => interactions.toggleHand(), [interactions]);

  const sendReaction = useCallback((emoji: ReactionEmoji): void => interactions.sendReaction(emoji), [interactions]);
