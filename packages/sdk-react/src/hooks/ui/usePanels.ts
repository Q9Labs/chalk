...
export function usePanels(): UsePanelsReturn {
  ...
  const openPanel = useCallback((panel: PanelType): void => ui.openPanel(panel), [ui]);

  const closePanel = useCallback((): void => ui.closePanel(), [ui]);

  const togglePanel = useCallback((panel: Exclude<PanelType, null>): void => ui.togglePanel(panel), [ui]);

  const showControls = useCallback((autoHideDelay?: number): void => ui.showControls(autoHideDelay), [ui]);
