...
export interface UseHapticsReturn {
	trigger: (
		input?: ChalkHapticInput,
		options?: ChalkHapticTriggerOptions,
	) => Promise<void>;
	cancel: () => void;
	isSupported: boolean;
	isEnabled: boolean;
...
) as readonly ChalkHapticPreset[];

export function useHaptics({
	enabled = true,
	respectReducedMotion = true,
}: UseHapticsOptions = {}): UseHapticsReturn {
	const prefersReducedMotion = usePrefersReducedMotion();
	const { trigger: rawTrigger, cancel: rawCancel, isSupported } = useWebHaptics();

	const isEnabled =
		enabled && (!respectReducedMotion || !prefersReducedMotion) && isSupported;
