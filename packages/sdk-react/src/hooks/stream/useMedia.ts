...
export function useMedia(): UseMediaReturn {
  ...
  const toggleVideo = useCallback((): Promise<boolean> => media.toggleVideo(), [media]);

  const toggleAudio = useCallback((): Promise<boolean> => media.toggleAudio(), [media]);

  const selectCamera = useCallback((deviceId: string): Promise<void> => media.selectCamera(deviceId), [media]);

  const applyBackgroundEffect = useCallback((effect: VideoBackgroundEffect): Promise<void> => media.applyBackgroundEffect(effect), [media]);

  const clearBackgroundEffect = useCallback((): Promise<void> => media.clearBackgroundEffect(), [media]);

  const selectMicrophone = useCallback((deviceId: string): Promise<void> => media.selectMicrophone(deviceId), [media]);

  const selectSpeaker = useCallback((deviceId: string): Promise<void> => media.selectSpeaker(deviceId), [media]);

  const refreshDevices = useCallback((): Promise<readonly MediaDevice[]> => media.refreshDevices(), [media]);

  const undoDeviceChange = useCallback((): void => media.undoDeviceChange(), [media]);
  ...
  return useMemo(
    ...
    [state, media, applyBackgroundEffect, clearBackgroundEffect, toggleVideo, toggleAudio, selectCamera, selectMicrophone, selectSpeaker, refreshDevices, undoDeviceChange],
  );
}
