...
export function useTranscription(options: UseTranscriptionOptions = {}): UseTranscriptionReturn {
  ...
  const exportTranscripts = useCallback(
    (format: "txt" | "srt" | "vtt"): string => {
      switch (format) {
        case "txt":
          return transcripts.map((t) => `[${t.timestamp.toLocaleTimeString()}] ${t.speaker}: ${t.text}`).join("\n");
  ...
  const searchTranscripts = useCallback(
    (query: string): TranscriptEntry[] => {
      if (!query.trim()) return transcripts;
      const lowerQuery = query.toLowerCase();
      return transcripts.filter((t) => t.text.toLowerCase().includes(lowerQuery) || t.speaker.toLowerCase().includes(lowerQuery));
    },
    [transcripts],
  );
