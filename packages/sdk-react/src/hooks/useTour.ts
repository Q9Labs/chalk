...
export function useTour(options: UseTourOptions): UseTourReturn {
  const { steps, onComplete, onSkip, onStepChange, storageKey = "chalk-tour-completed" } = options;
  ...
  const currentStepData = isOpen && currentStep < totalSteps ? (steps[currentStep] ?? null) : null;
...
export const DEFAULT_MEETING_TOUR_STEPS: TourStep[] = [
  {
    ...
    title: "Video Grid",
    description: "This is where you'll see all meeting participants. Click any tile to pin it.",
    placement: "bottom",
    spotlight: true,
  },
  {
    ...
    title: "Your Video",
    description: "Your camera preview. Others can see you when your video is on.",
    placement: "bottom",
    spotlight: true,
  },
  {
