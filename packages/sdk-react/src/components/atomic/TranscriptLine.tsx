...
export const TranscriptLine = React.memo<TranscriptLineProps>(({
  ...
  const timeString = useMemo(() => new Intl.DateTimeFormat('en-US', {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false,
  ...
  return (
    <div
      className={cn(
        ...
        !prefersReducedMotion && !isInterim && 'chalk-animate-transcript-in',
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            className={cn(
              ...
              isInterim ? 'text-muted-foreground/70' : 'text-foreground',
              isLowConfidence && !isInterim && 'chalk-low-confidence'
            )}
            title={isLowConfidence && !isInterim ? `Low confidence: ${confidencePercent}%` : undefined}
          >
            {renderedText}
