...
import { ArrowLeft01Icon, ArrowRight01Icon, MoreVerticalIcon } from "../../utils/icons";
import { Tooltip, TooltipContent, TooltipTrigger } from "../ui";

const { Group, Panel, Separator, useDefaultLayout, useGroupRef, usePanelRef } = resizablePanelsPkg as any;
...
export function SplitStage({ leftPanel, rightPanel, className, onLayoutChange }: SplitStageProps) {
  ...
  return (
    <Group orientation="horizontal" className={cn("h-full w-full", className)} defaultLayout={defaultLayout} onLayoutChanged={handleLayoutChanged} groupRef={groupRef}>
      <Panel id={leftPanelId} panelRef={leftPanelRef} defaultSize="50%" minSize="0%" collapsible={true} onResize={(size) => setIsLeftCollapsed(size.asPercentage <= 0)} className={cn("relative transition-all duration-300 ease-in-out", isLeftCollapsed && "min-w-[0px] w-[0px] overflow-hidden")}>
        {leftPanel}
      </Panel>
      ...
      <Panel id={rightPanelId} panelRef={rightPanelRef} defaultSize="50%" minSize="0%" collapsible={true} onResize={(size) => setIsRightCollapsed(size.asPercentage <= 0)} className={cn("relative transition-all duration-300 ease-in-out", isRightCollapsed && "min-w-[0px] w-[0px] overflow-hidden")}>
        {rightPanel}
      </Panel>
    </Group>
  );
...
function DocketControl({ isLeftCollapsed, isRightCollapsed, onMaximizeLeft, onMaximizeRight, onReset }: { isLeftCollapsed: boolean; isRightCollapsed: boolean; onMaximizeLeft: () => void; onMaximizeRight: () => void; onReset: () => void }) {
  return (
    <div
      className={cn(
