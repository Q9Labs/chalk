...
export function useConferenceMeetingActions({
  ...
  }, [setShowLeaveConfirm, setIsExiting, clearDisconnectGraceTimeout, setIsDisconnectGraceActive, leave, play, onEnd, buildEndData, setPhase, onLeave]);
  ...
  const handleRejoin = useCallback(() => {
    ...
    setSupportCode(null);
    resetForRejoin();
  }, [clearDisconnectGraceTimeout, setIsDisconnectGraceActive, setPhase, setSupportCode, resetForRejoin]);
  ...
  const handleSendMessageWithAttachments = useCallback((content: string, files: File[]) => sendChatMessageWithAttachments(content, files), [sendChatMessageWithAttachments]);
