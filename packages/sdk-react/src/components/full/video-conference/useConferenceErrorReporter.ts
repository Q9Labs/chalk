...
export function useConferenceErrorReporter({
	session,
	onError,
	roomIdRef,
	phaseRef,
	pushIncidentBreadcrumb,
}: UseConferenceErrorReporterParams): UseConferenceErrorReporterReturn {
	...
	const emitError = useCallback(
		(errorToReport: ChalkError, details?: Record<string, unknown>) => {
			...
			const errorSupportCode =
				typeof errorToReport.details?.supportCode === "string"
					? errorToReport.details.supportCode
					: undefined;
