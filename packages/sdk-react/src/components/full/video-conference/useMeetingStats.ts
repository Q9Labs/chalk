...
export function useMeetingStats({ phase, roomId, participants, participantCount, messagesLength, committedTranscripts, recordingId, recordingDurationSeconds, isLocalScreenSharing, isWhiteboardOpen, activeReactionCount }: UseMeetingStatsParams): UseMeetingStatsReturn {
  ...
  const buildEndData = useCallback((): MeetingEndData => {
    ...
    const hostSession = participantSessions.find((participantSession) => participantSession.role === "host");
  ...
  }, [roomId, meetingDuration, participantCount, committedTranscripts, recordingId, messagesLength, recordingDurationSeconds]);
