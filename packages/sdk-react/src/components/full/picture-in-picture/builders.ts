...
export function buildPreJoinPictureInPictureSource({
	displayName,
	videoTrack,
	isAudioEnabled,
	isVideoEnabled,
...
export function buildMeetingPictureInPictureSource({
	...
	const screenSharer = participants.find(
		(participant) =>
			participant.isScreenSharing && hasLiveTrack(participant.screenShareTrack),
	);
	...
	if (screenSharer) {
		return {
			...
			previewSource:
				localParticipant.id !== screenSharer.id
					? buildParticipantSource(localParticipant)
					: null,
		};
	}
	...
	const activeSpeaker = participants.find(
		(participant) =>
			...
			hasLiveTrack(participant.videoTrack),
	);
	const firstRemoteVideo = participants.find(
		(participant) =>
			!participant.isLocal &&
			participant.isVideoEnabled &&
			hasLiveTrack(participant.videoTrack),
	...
	const localVideo =
		localParticipant.isVideoEnabled && hasLiveTrack(localParticipant.videoTrack)
			? localParticipant
			: null;
	const fallbackParticipant =
		activeSpeaker ?? firstRemoteVideo ?? localVideo ?? participants[0] ?? localParticipant;

	const source = buildParticipantSource(fallbackParticipant);
	const previewSource =
		localVideo && source?.id !== localParticipant.id
			? buildParticipantSource(localParticipant)
			: null;
...
function buildParticipantSource(
	...
	return {
		...
		videoTrack:
			participant.isVideoEnabled && hasLiveTrack(participant.videoTrack)
