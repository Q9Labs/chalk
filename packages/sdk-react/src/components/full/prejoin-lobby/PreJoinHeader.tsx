  ...
  return (
    <div
      className={cn(
        ...
        isMobile
          ? "px-4 py-3"
          : "px-6 lg:px-8 py-6 max-w-6xl mx-auto md:items-center"
      )}
    >
      <div className={cn("flex items-center min-w-0", isMobile ? "gap-2" : "gap-3 md:gap-4")}>
        <img
          src="/brand/chalk/chalk-logo.svg"
          alt="Chalk"
          className={cn("block shrink-0", isMobile && "invert")}
          style={logoStyle}
          draggable={false}
        />
        {roomName && !isMobile && (
      ...
      <button
        ...
        className={cn(
          "rounded-full flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95",
          isMobile
            ? "w-10 h-10 bg-white/20 text-white hover:bg-white/30 shadow-lg"
            : "w-11 h-11 hover:bg-muted text-foreground shadow-sm hover:shadow-md"
        )}
      >
