...
export function usePreJoinMedia({
	...
	onAudioUnavailable,
	videoRef,
}: UsePreJoinMediaParams): UsePreJoinMediaReturn {
	...
	const effectiveVideoDevices = useMemo(
		() => (videoDevices.length > 0 ? [...videoDevices] : localVideoDevices),
	...
	const effectiveAudioInputDevices = useMemo(
		...
		[audioInputDevices, localAudioInputDevices],
	);
