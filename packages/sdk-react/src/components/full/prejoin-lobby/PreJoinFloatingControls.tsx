  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-3 touch-manipulation" ref={dropdownRef}>
      ...
      {openDropdown && (
        ...
        >
          <div className="px-3 py-3 text-xs font-semibold text-white/60 uppercase tracking-wide border-b border-white/10">{openDropdown === "audio" ? "Microphone" : "Camera"}</div>
            ...
            {dropdownDevices.map((device) => {
              ...
              return (
                <button
                  ...
                  className={cn("w-full px-3 py-2.5 text-left text-sm transition-colors flex items-center gap-3 outline-none focus-visible:bg-white/10", isSelected ? "bg-[var(--primary)]/20 text-[var(--primary)]" : "text-white/90 hover:bg-white/10")}
                >
                  {isSelected && <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] shrink-0" />}
          ...
          {hasAudioInput && (
            <button
              ...
              className={cn(
                ...
                openDropdown === "audio" ? "text-[var(--primary)]" : "text-(--muted-foreground) hover:text-(--foreground) hover:bg-black/5 dark:hover:bg-white/5",
              )}
            >
        ...
        <div className="flex items-center gap-1">
          ...
          {hasVideoDevices && (
            <button
              ...
              className={cn(
                "w-8 h-11 rounded-r-full -ml-1 flex items-center justify-center transition-all duration-200 outline-none focus-visible:bg-[var(--primary)]/10",
                openDropdown === "video" ? "text-[var(--primary)]" : "text-(--muted-foreground) hover:text-(--foreground) hover:bg-black/5 dark:hover:bg-white/5",
              )}
            >
        ...
        {enablePictureInPicture && isPictureInPictureSupported && onTogglePictureInPicture ? (
          <button
            ...
            className={cn(
              ...
              isPictureInPictureActive ? "bg-[var(--primary)] text-white" : "bg-black/5 dark:bg-white/10 text-(--foreground) hover:bg-black/10 dark:hover:bg-white/20",
            )}
          >
