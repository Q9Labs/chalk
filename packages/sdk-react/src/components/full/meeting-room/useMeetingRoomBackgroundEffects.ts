...
import { loadLocalBackgroundAsset, persistLocalBackgroundAsset } from "../../../utils/localBackgroundAssetStore";
import { areVideoBackgroundEffectsEqual, getStoredVideoBackgroundEffectId, getVideoBackgroundPreset, toRuntimeVideoBackgroundEffect, VIDEO_BACKGROUND_BLUR_ID, VIDEO_BACKGROUND_CUSTOM_ID, VIDEO_BACKGROUND_PRESETS, type StoredVideoBackgroundEffect } from "../../../utils/videoBackgrounds";
...
const applyAsync = async (task?: (() => Promise<void> | void) | undefined) => {
  if (!task) {
    return;
  }
...
export const useMeetingRoomBackgroundEffects = ({ enabled, settings, updateVideoSettings, currentEffect, isSupported, isApplying, applyBackgroundEffect, clearBackgroundEffect }: UseMeetingRoomBackgroundEffectsParams) => {
  ...
  useEffect(() => {
    if (!backgroundEffect || backgroundEffect.type !== "custom" || !backgroundEffect.assetKey) {
  ...
  const selectedEffectId = useMemo(() => getStoredVideoBackgroundEffectId(backgroundEffect), [backgroundEffect]);
  ...
  const syncRuntimeEffect = useCallback(
    ...
    [applyBackgroundEffect, clearBackgroundEffect, currentEffect, enabled, isSupported],
  );
  ...
  }, [backgroundEffect, customPreviewUrl, enabled, syncRuntimeEffect]);
  ...
  const handleSelect = useCallback(
    ...
    [backgroundEffect, customPreviewUrl, syncRuntimeEffect, updateVideoSettings],
  );
