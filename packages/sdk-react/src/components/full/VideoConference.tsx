...
function MobileAppRedirectScreen({ error, userName }: { error: string | null; userName: string }): React.JSX.Element {
  if (!error) {
    return <LoadingScreen message="Opening Chalk..." displayName={userName} supportingMessages={["We’re handing this meeting off to the mobile app."]} />;
  }
  ...
  return (
    <div data-chalk className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-3xl border border-border/60 bg-card/90 p-6 shadow-2xl backdrop-blur-sm space-y-4 text-center">
        ...
        <h1 className="text-2xl font-semibold leading-tight">Chalk needs to open in the mobile app.</h1>
...
function VideoConferenceBase(props: VideoConferenceProps): React.JSX.Element {
  ...
  if (mobileRedirect.isBlocking) {
    return <MobileAppRedirectScreen error={mobileRedirect.status === "failed" ? mobileRedirect.error : null} userName={props.userName} />;
  }
