...
export const MessageBubble = React.memo<MessageBubbleProps>(({
      ...
      if (part.match(URL_REGEX)) {
        return (
          <a
            ...
            className={cn("underline break-all", isLocal ? "text-primary-foreground" : "text-primary")}
          >
            {part}
          </a>
        );
          ...
          if (isImage && imageUrl) {
            return (
              <button
                ...
                className="relative group overflow-hidden rounded-lg border border-border/50 max-w-full sm:max-w-xs aspect-auto"
              >
                <img
                  src={imageUrl}
                  alt={file.fileName}
            ...
            <button
              ...
              className={cn(
                ...
                isLocal
                  ...
                  : "bg-muted/50 border-border/50 hover:bg-muted/80 text-foreground"
              )}
            >
              <div className={cn(
                "w-10 h-10 rounded-lg flex items-center justify-center shrink-0",
                isLocal ? "bg-primary-foreground/20" : "bg-primary/10 text-primary"
              )}>
  ...
  const renderStatus = () => {
    ...
    const statusTitle =
      readByCount > 0
        ? `Read by: ${readBy.map((receipt) => receipt.displayName).join(', ')}`
        : statusLabel;
    ...
    return (
      <div className="flex items-center gap-1 group/status relative" title={statusTitle}>
        ...
        <span className={cn("text-[11px]", isRead ? "text-primary" : "text-muted-foreground")}>
          {statusLabel}
        </span>

        {isLocal && readByCount > 0 && (
          <div className="invisible group-hover/status:visible absolute bottom-full right-0 mb-2 whitespace-nowrap bg-popover text-popover-foreground px-2 py-1 rounded text-[10px] shadow-lg border border-border z-10">
            Read by: {readBy.map(r => r.displayName).join(', ')}
          </div>
        )}
      </div>
    );
