...
const buildPathData = (points: { x: number; y: number }[]) => {
  ...
  return points
    .map((point, index) => `${index === 0 ? "M" : "L"} ${toUnit(point.x)} ${toUnit(point.y)}`)
    .join(" ");
};
...
const renderItem = (item: ScreenAnnotationItem) => {
  ...
  if (item.type === "freehand") {
    return (
      <path
        key={item.id}
        d={buildPathData(item.points)}
        fill="none"
        stroke={item.style.color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeOpacity={item.style.opacity ?? (item.tool === "highlighter" ? 0.35 : 1)}
        strokeWidth={item.style.strokeWidth * 6}
      />
    );
  }
  ...
  if (item.type === "shape") {
    ...
    return (
      <line
        ...
        x2={endX}
        y2={endY}
        markerEnd={item.shape === "arrow" ? "url(#chalk-annotation-arrow)" : undefined}
  ...
  return (
    <text
      key={item.id}
      x={toUnit(item.position.x)}
      y={toUnit(item.position.y)}
      fill={item.style.color}
      fillOpacity={item.style.opacity ?? 1}
...
export const ScreenAnnotationsSvg = memo(
  ({
    items,
    draftItem,
    interactive,
