import type { AnnotationAccessMode, ScreenAnnotationItem, ScreenAnnotationCursor, ScreenAnnotationTool } from "@q9labs/chalk-core";
...
const isShapeTool = (tool: ScreenAnnotationTool): tool is ShapeTool => tool === "rectangle" || tool === "ellipse" || tool === "line" || tool === "arrow";

const getStyleForTool = (tool: ScreenAnnotationTool) => ({
  color: tool === "highlighter" ? "#facc15" : tool === "text" ? "#f8fafc" : "#22d3ee",
...
const replaceOrAppend = (items: readonly ScreenAnnotationItem[], nextItem: ScreenAnnotationItem) => {
...
export const ScreenAnnotationsLayer = memo(({ enabled, className }: ScreenAnnotationsLayerProps) => {
  ...
  const cursorChips = useMemo(() => cursors.filter((cursor: ScreenAnnotationCursor) => cursor.participantId !== localParticipant?.id), [cursors, localParticipant?.id]);
