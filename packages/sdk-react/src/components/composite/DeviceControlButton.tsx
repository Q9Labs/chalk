...
export const DeviceControlButton = ({
  ...
  const icon = isMic ? (
    isActive ? <Microphone01Icon /> : <MicrophoneOff01Icon className="text-[#dc2626]" />
  ) : (
    isActive ? <Video01Icon /> : <VideoOffIcon className="text-[#dc2626]" />
  );
  ...
  return (
    <div className={cn("relative flex items-center", className)} ref={containerRef}>
      ...
      <ControlButton
        ...
        haptic={haptic}
        className={cn(
          "rounded-r-none border-r border-black/5 dark:border-white/5",
      ...
      {isOpen && (
        <div 
          className={cn(
            "absolute z-50 min-w-[240px] max-w-[280px] rounded-2xl border border-black/10 dark:border-white/10 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl shadow-2xl overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200",
            orientation === 'up' ? "bottom-full mb-3" : "top-full mt-3",
            "left-0"
          )}
        >
          <div className="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 border-b border-black/5 dark:border-white/5 mb-1">
            {dropdownLabel}
          </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? "bg-[var(--primary)]/10 text-[var(--primary)] font-medium" : "text-zinc-700 dark:text-zinc-300"
                    )}
                  >
                    <div className="flex items-center gap-3 truncate pr-2">
                       <span className={cn(
                          "h-2 w-2 shrink-0 rounded-full",
                          isSelected ? "bg-[var(--primary)]" : "bg-black/10 dark:bg-white/10"
                        )} />
          ...
          {secondaryDevices && secondaryDevices.length > 0 && (
            <>
              <div className="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 border-y border-black/5 dark:border-white/5 my-1 bg-black/5 dark:bg-white/5">
                Speakers
              </div>
