  ...
  return (
    <div className={cn("flex flex-col w-80 overflow-hidden rounded-lg shadow-lg", "bg-card", "border border-border/50", className)} role="complementary" aria-label="Waiting room">
      ...
      <div className="p-2 border-b border-border/50 flex gap-2">
        {onAdmitAll && (
          <button onClick={onAdmitAll} disabled={participants.length === 0} className={cn("flex-1 px-3 py-1.5 rounded text-xs font-medium transition-colors", "bg-green-600 text-white hover:bg-green-700", "disabled:opacity-50 disabled:cursor-not-allowed")}>
            Admit All
          </button>
        )}
        {onDenyAll && (
          <button onClick={onDenyAll} disabled={participants.length === 0} className={cn("flex-1 px-3 py-1.5 rounded text-xs font-medium transition-colors", "bg-secondary text-destructive", "hover:bg-accent", "disabled:opacity-50 disabled:cursor-not-allowed")}>
            Deny All
          </button>
        )}
      </div>
      ...
      <div className="max-h-80 overflow-y-auto p-2 space-y-1">
        {participants.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">No one is waiting</div>
        ) : (
          participants.map((p) => (
            <div key={p.id} className={cn("flex items-center justify-between p-2 rounded-md transition-colors", "hover:bg-secondary")}>
              <div className="flex items-center gap-3 min-w-0">
                <Avatar src={p.avatarUrl} name={p.displayName} size="sm" />
                <div className="flex flex-col min-w-0">
                  <span className="text-sm font-medium truncate text-card-foreground">{p.displayName}</span>
                  <span className="text-xs text-muted-foreground">Waiting for {getWaitingTime(p.joinedAt)}</span>
                </div>
              </div>
