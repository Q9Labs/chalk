...
function VideoCall() {
  ...
  return (
    <div>
      <button onClick={() => joinWithInviteLink("https://chalkmeet.com/j/join-token-123", { userName: "John Doe" })}>Join invite</button>
...
- On iPhone, iPad, and Android, turnkey `VideoConference` now redirects into the native Chalk app before rendering mobile web lobby UI.
- iOS now falls back to Chalk’s production App Store listing by default.
- Pass `mobileRedirect.iosStoreUrl` only when you need to override that default.
- Custom SDK surfaces can reuse the same logic through `useMobileAppRedirect()`.
