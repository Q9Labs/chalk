import type { Collaborator, ExcalidrawImperativeAPI } from "./types";
...
export class WhiteboardPresence {
	...
	handleRemoteCursor(payload: {
		...
		this.collaborators.set(payload.participantId, {
			...
			socketId: payload.participantId as any,
			_ts: ts,
		});
	...
	private apply() {
		...
		this.opts.excalidrawAPI.updateScene({ collaborators: map } as any);
	}
}
