...
export class ExcalidrawCollabEngine {
  ...
  handleChange(_elements: readonly OrderedExcalidrawElement[], _appState: AppState, files: any): void {
  ...
  private applyRemoteElements(args: { sceneId: string | null; syncAll: boolean; remoteElements: unknown[]; appState?: AppState; isSnapshot: boolean }) {
    ...
    const reconciled = reconcileElements(local as any, restoredRemote as any, excalidrawAPI.getAppState());
