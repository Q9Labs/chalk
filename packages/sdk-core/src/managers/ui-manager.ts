...
/** Side panel types */
export type PanelType = "chat" | "participants" | "settings" | "whiteboard" | null;
...
export class UIManager extends StateContainer<UIState> {
  ...
  private notificationTimeouts = new Map<string, ReturnType<typeof setTimeout>>();
  ...
  on<K extends keyof UIManagerEvents>(event: K, handler: (data: UIManagerEvents[K]) => void): () => void {
  ...
  /** Add a notification */
  notify(message: string, severity: NotificationSeverity = "info", autoDismiss = true): string {
