...
export type ChalkIncidentSource =
	| "session"
	| "video_conference"
	| "room"
	| "media"
...
export type IncidentReporter =
	| ((incident: ChalkIncident) => Promise<void>)
	| ((incident: ChalkIncident) => void);
...
const canUseSendBeacon = (): boolean =>
 ...
 */
export const createSupportCode = (
	sequence: number,
	now: Date = new Date(),
): string => {
	const datePart = [
	...
	const seqPart = Math.max(1, sequence)
		.toString(36)
		.toUpperCase()
		.padStart(3, "0");
 ...
 */
export const createHttpIncidentReporter = (
	config: HttpIncidentReporterConfig,
): IncidentReporter => {
	...
	const fetchImpl =
		config.fetchImpl ?? (typeof fetch === "function" ? fetch : undefined);
	...
	return async (incident: ChalkIncident): Promise<void> => {
		...
		for (let attempt = 0; attempt <= retries; attempt += 1) {
			...
			try {
				...
				if (!response.ok) {
					throw new Error(
						`incident reporter request failed: ${response.status} ${response.statusText}`,
					);
				}
