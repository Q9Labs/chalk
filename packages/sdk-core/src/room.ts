...
import { getRealtimeKitDiagnosticsSnapshot, type RealtimeKitDiagnosticsSnapshot } from "./debug/rtk-diagnostics.ts";
...
export interface ConferenceSessionDiagnosticsSnapshot {
  roomId: string;
  ...
  rtk: RealtimeKitDiagnosticsSnapshot;
}
...
export class ConferenceSession extends EventEmitter<ConferenceSessionEvents> {
  ...
  getDiagnosticsSnapshot(): ConferenceSessionDiagnosticsSnapshot {
    return {
      roomId: this.id,
      connectionState: this.connectionState,
      hasRtkClient: Boolean(this.rtkClient),
      hasWsClient: Boolean(this.wsClient),
      localParticipantId: this.sessionStore.getLocalParticipant()?.id ?? null,
      participantCount: this.sessionStore.getParticipants().size,
      rtk: getRealtimeKitDiagnosticsSnapshot(this.rtkClient),
    };
  }
  ...
  private setupRTKListeners(): void {
    setupConferenceSessionRtkSignaling({
      ...
      hasSelectedBackgroundEffect: () => this.mediaController.hasSelectedBackgroundEffect(),
      reapplyBackgroundEffect: () => this.mediaController.reapplyBackgroundEffect(),
