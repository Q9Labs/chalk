...
const hasMiddlewareApis = (
	rtkClient: RealtimeKitClient | undefined,
): rtkClient is RealtimeKitClient & { self: VideoMiddlewareCapableSelf } => {
	if (!rtkClient) {
		return false;
	}
	...
	return (
		typeof self.addVideoMiddleware === "function" &&
		typeof self.removeAllVideoMiddlewares === "function"
	);
};

export const isConferenceSessionVideoBackgroundSupported = (
	...
	return (
		...
		hasMiddlewareApis(rtkClient)
	);
};

export const createConferenceSessionVideoBackgroundController = (deps: {
	...
	const removeAllVideoMiddlewares = async (
		rtkClient: RealtimeKitClient & { self: VideoMiddlewareCapableSelf },
	) => {
	...
	const applyBackgroundEffect = async (
		...
		const middleware =
			effect.mode === "blur"
				? await resolvedTransformer.createBackgroundBlurVideoMiddleware(
						effect.blurStrength ?? DEFAULT_BLUR_STRENGTH,
					)
				: await resolvedTransformer.createStaticBackgroundVideoMiddleware(
						effect.imageUrl,
					);
