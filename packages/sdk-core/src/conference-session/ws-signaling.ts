...
export const setupConferenceSessionWsSignaling = (deps: WsSignalingDeps): (() => void) => {
  ...
  subscribe("room.snapshot", (snapshot) => {
    ...
    if (deps.hasRtkClient()) {
      // Native RTK joins can report a websocket snapshot before the RTK roomJoined
      // callback updates the shared room state. Treat the snapshot as proof that the
      // room is live so session bridges can promote out of "joining" instead of
      // waiting forever on a stale disconnected status.
      deps.setConnectionState("connected");
