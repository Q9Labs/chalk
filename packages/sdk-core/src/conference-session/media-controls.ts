...
export const createConferenceSessionMediaController = (deps: MediaControllerDeps) => {
  ...
  const startScreenShare = async (options?: ScreenShareOptions): Promise<boolean> => {
    ...
    try {
      ...
      deps.emitParticipantUpdated(localParticipant.id, localParticipant);
      const trackDiagnostics = getScreenShareTrackDiagnostics(localParticipant.screenShareTrack, localParticipant.screenShareAudioTrack);
      ctx.complete("success", trackDiagnostics);

      if (isDevRuntime()) {
        console.info("[chalk][screenshare] start:success", {
          participantId: localParticipant.id,
          ...trackDiagnostics,
        });
      }
      ...
      if (isDevRuntime()) {
        console.warn("[chalk][screenshare] start:failure", {
          participantId: localParticipant.id,
          errorName: name ?? null,
          errorMessage: message,
          isCancelled,
