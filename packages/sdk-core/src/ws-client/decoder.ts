...
const truncate = (raw: string) =>
	raw.length > MAX_RAW_LEN ? `${raw.slice(0, MAX_RAW_LEN)}…` : raw;

const isInboundType = (type: string): type is WSInboundType =>
	Object.prototype.hasOwnProperty.call(WSPayloadSchemas, type);

const decodeEnvelope = Schema.decodeUnknownSync(WSMessage);

const decodePayload = <K extends WSInboundType>(
	type: K,
	payload: unknown,
): WSInboundPayloadMap[K] => {
...
export const decodeIncomingMessage = (raw: string) => {
	...
	const payload =
		envelope.payload === undefined
			? undefined
			: envelope.type === "whiteboard.data" ||
					envelope.type === "whiteboard.snapshot"
