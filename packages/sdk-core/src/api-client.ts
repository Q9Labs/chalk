...
export class APIClient extends EventEmitter<APIClientEvents> {
  ...
  private async request<T>(method: string, path: string, body?: unknown, isRetry = false, options?: { skipAuth?: boolean }): Promise<ApiResponse<T>> {
    ...
    try {
      ...
      const data = rawData && typeof rawData === "object" ? snakeToCamel<T>(rawData as T) : (undefined as T | undefined);
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData?.message ?? errorData?.error ?? fallbackMessage;
  ...
  private async handle401<T>(method: string, path: string, body?: unknown, options?: { skipAuth?: boolean }): Promise<ApiResponse<T>> {
    if (!this.tokenProvider) {
  ...
  async createRoom(options: CreateRoomOptions = {}): Promise<ApiResponse<RoomResource>> {
  ...
  async scheduleRoom(options: ScheduleRoomOptions): Promise<ApiResponse<RoomResource>> {
    const response = await this.request<RoomResource>("POST", "/api/v1/rooms/schedule", {
      name: options.name,
      config: options.config,
      scheduledStartAt: options.scheduledStartAt instanceof Date ? options.scheduledStartAt.toISOString() : options.scheduledStartAt,
      scheduledEndAt: options.scheduledEndAt instanceof Date ? options.scheduledEndAt.toISOString() : options.scheduledEndAt,
  ...
  async createSession(name?: string, config?: Record<string, unknown>): Promise<ApiResponse<CreateRoomResponse>> {
  ...
  async exchangeJoinToken(joinToken: string): Promise<ApiResponse<ExchangeJoinTokenResponse>> {
    return this.request<ExchangeJoinTokenResponse>("POST", "/api/v1/public/join-token/exchange", { joinToken }, false, { skipAuth: true });
  }
  ...
  async addParticipant(roomId: string, displayName: string, role?: "host" | "participant", metadata?: Record<string, unknown>): Promise<ApiResponse<JoinSessionResponse>> {
    const response = await this.request<TransformedJoinSessionApiResponse>("POST", `/api/v1/rooms/${roomId}/participants`, {
      displayName,
      role,
      metadata,
    });
  ...
  async demoJoin(roomId: string, displayName: string): Promise<ApiResponse<JoinSessionResponse>> {
    const response = await this.request<TransformedJoinSessionApiResponse>("POST", "/api/v1/demo/join", {
      roomId,
      displayName,
    });
  ...
  private transformJoinResponse(data: TransformedJoinSessionApiResponse): JoinSessionResponse {
    ...
    if (!accessToken) {
      throw new ChalkError(ChalkErrorCode.AUTH_FAILED, "Missing access token in join response - server did not provide valid authentication");
    }
  ...
  private normalizeRoomResource(data: RoomResource & Partial<CreateRoomResponse>): RoomResource {
    const id = data.id ?? data.roomId;
    if (!id) {
      throw new ChalkError(ChalkErrorCode.INVALID_PARAMS, "Missing room ID in room response");
    }
  ...
  async removeParticipant(roomId: string, participantId: string): Promise<ApiResponse<void>> {
    return this.request<void>("DELETE", `/api/v1/rooms/${roomId}/participants/${participantId}`);
  }

  async updateParticipant(roomId: string, participantId: string, data: { displayName?: string; role?: string }): Promise<ApiResponse<RoomResource["activeParticipantCount"]>> {
    return this.request<RoomResource["activeParticipantCount"]>("PATCH", `/api/v1/rooms/${roomId}/participants/${participantId}`, data);
  }

  // Recording endpoints
  async startRecording(roomId: string): Promise<ApiResponse<{ recordingId: string }>> {
    return this.request<{ recordingId: string }>("POST", `/api/v1/rooms/${roomId}/recordings/start`);
  }

  async stopRecording(roomId: string): Promise<ApiResponse<Recording>> {
    return this.request<Recording>("POST", `/api/v1/rooms/${roomId}/recordings/stop`);
  }
  ...
  async getRecordingDownloadUrl(recordingId: string): Promise<ApiResponse<{ url: string }>> {
    return this.request<{ url: string }>("GET", `/api/v1/recordings/${recordingId}/download`);
  }

  // Whiteboard file presign (R2)
  async presignWhiteboardUpload(roomId: string, fileId: string, mimeType: string): Promise<ApiResponse<{ uploadUrl: string; expiresAtMs: number }>> {
    return this.request<{ uploadUrl: string; expiresAtMs: number }>("POST", `/api/v1/rooms/${roomId}/whiteboard/files/presign-upload`, { fileId, mimeType });
  }

  async presignWhiteboardDownload(roomId: string, fileId: string): Promise<ApiResponse<{ downloadUrl: string; expiresAtMs: number }>> {
    return this.request<{ downloadUrl: string; expiresAtMs: number }>("POST", `/api/v1/rooms/${roomId}/whiteboard/files/presign-download`, { fileId });
  }
  ...
  > {
    ...
    }>("POST", `/api/v1/rooms/${roomId}/chat/attachments/presign-upload`, { files });
  }

  async presignChatAttachmentDownload(roomId: string, attachmentId: string): Promise<ApiResponse<{ downloadUrl: string; expiresAtMs: number }>> {
    return this.request<{ downloadUrl: string; expiresAtMs: number }>("POST", `/api/v1/rooms/${roomId}/chat/attachments/presign-download`, { attachmentId });
  }

  async uploadChatAttachment(roomId: string, attachmentId: string, file: File): Promise<ApiResponse<void>> {
    const ctx = wideEvents.start("api.request");
    ctx.set("request", {
      method: "POST",
