...
import { type ChalkError, ChalkErrorCode, type Err, err, type Ok, ok, type Result } from "../types.ts";
...
describe("ChalkErrorCode", () => {
  describe("Error codes structure", () => {
    it("should have network error codes", () => {
      ...
      expect(ChalkErrorCode.MAX_RECONNECT_ATTEMPTS).toBe("MAX_RECONNECT_ATTEMPTS");
    ...
    it("should have media error codes", () => {
      ...
      expect(ChalkErrorCode.MICROPHONE_ACCESS_DENIED).toBe("MICROPHONE_ACCESS_DENIED");
      ...
      expect(ChalkErrorCode.SCREEN_SHARE_CANCELLED).toBe("SCREEN_SHARE_CANCELLED");
    });
  ...
  describe("Error code type safety", () => {
    it("should be assignable to ChalkErrorCode type", () => {
      const code: (typeof ChalkErrorCode)[keyof typeof ChalkErrorCode] = ChalkErrorCode.CAMERA_ACCESS_DENIED;
...
describe("ChalkError interface", () => {
  ...
  it("should support additional details", () => {
    ...
    expect(error.details?.availableDevices).toEqual(["device_456", "device_789"]);
  });
