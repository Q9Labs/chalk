...
class MockRoom extends EventEmitter {
  ...
  private readonly mutateParticipantOnStart: boolean;

  constructor(localParticipant: ParticipantLike, options?: { emitParticipantUpdatedOnStart?: boolean; startResult?: boolean; mutateParticipantOnStart?: boolean }) {
    super();
    ...
    this.mutateParticipantOnStart = options?.mutateParticipantOnStart ?? true;
  }

  async startScreenShare(): Promise<boolean> {
    if (!this.localParticipant || !this.startResult) return false;
    if (this.mutateParticipantOnStart) {
      this.localParticipant.isScreenSharing = true;
      this.localParticipant.screenShareTrack = createTrack();
    }
    if (this.emitParticipantUpdatedOnStart && this.mutateParticipantOnStart) {
...
describe("ScreenShareManager", () => {
  ...
  it("does not mark the local user as sharing when start returns before room state confirms it", async () => {
    const room = new MockRoom(
      {
        id: "local",
        isLocal: true,
      ...
      { startResult: true, mutateParticipantOnStart: false },
    );
  ...
  it("does not mark local screen share active when the participant says sharing without a live track", async () => {
    const local = {
      id: "local",
      isLocal: true,
      isScreenSharing: false,
    };
    const room = new MockRoom(local);
  ...
  it("activates once a live local screen share track arrives after a stale enabled update", async () => {
    const local = {
      id: "local",
      isLocal: true,
      isScreenSharing: false,
