...
import { WebhookPayloadError, WebhookSignatureError, WebhookTimestampError } from "./errors";
 ...
 */
function verifyEffect(body: string, signature: string, timestamp: string, secret: string, tolerance: number) {
  return Effect.gen(function* () {
    ...
    yield* Effect.logDebug("Webhook timestamp verified").pipe(Effect.annotateLogs({ timestamp, drift }));
...
export function createWebhookHandler(options: WebhookHandlerOptions) {
  ...
  return {
     ...
     */
    verify(body: string, signature: string, timestamp: string): Promise<WebhookEvent> {
      return Effect.runPromise(
        verifyEffect(body, signature, timestamp, secret, tolerance).pipe(
          Effect.catchTags({
            WebhookTimestampError: (e) => Effect.fail(new ChalkError(ChalkErrorCode.WEBHOOK_TIMESTAMP_EXPIRED, e.message)),
            WebhookSignatureError: (e) => Effect.fail(new ChalkError(ChalkErrorCode.WEBHOOK_SIGNATURE_INVALID, e.message)),
            WebhookPayloadError: (e) => Effect.fail(new ChalkError(ChalkErrorCode.WEBHOOK_PAYLOAD_INVALID, e.message, { cause: e.cause instanceof Error ? e.cause : undefined })),
          }),
        ),
      );
    },
