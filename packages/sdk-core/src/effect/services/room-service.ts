...
import type { RoomEvent, RoomState, RoomStatus } from "../schemas/manager-state";
...
export const RoomServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Room event cleanup function ref
    ...
    const setupRoomListeners = (room: Room) =>
      Effect.sync(() => {
        const unsubStatus = room.on("status-changed", (status) => {
          Effect.runSync(
            Effect.gen(function* () {
              yield* SubscriptionRef.update(stateRef, (s) => ({ ...s, status: status as RoomStatus }));
              yield* PubSub.publish(eventBus, { _tag: "StatusChanged", status: status as RoomStatus });
