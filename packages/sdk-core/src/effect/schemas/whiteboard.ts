...
export const AppStateSchema = Schema.declare((input): input is AppState => isObject(input));

// For outbound deltas we often send partial appState.
export const AppStatePartialSchema = Schema.declare((input): input is Partial<AppState> => isObject(input));
