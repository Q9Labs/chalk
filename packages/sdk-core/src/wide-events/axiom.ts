...
export function createAxiomWideEventsHandler(config: AxiomWideEventsConfig): AxiomWideEventsHandler {
  ...
  if (isBrowser() && debug) {
    ...
    console.warn("[Chalk] Axiom wide-events handler is running in the browser; token is client-exposed");
  }
  ...
  const flushBatch = async (batch: WideEvent[]): Promise<void> => {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Axiom ingest failed: ${res.status} ${res.statusText} ${text}`.trim());
    }
  };
