...
import { createRealtimeKitInitEffect, emitRtkJoinAttemptTelemetry, isRetryableRtkJoinError, joinRealtimeKitWithRetry, preloadRealtimeKitBundle, type RetryableRealtimeKitClient } from "./conference-client/join-engine.ts";
...
import { ConferenceSession, type ConferenceSessionDiagnosticsSnapshot } from "./room.ts";
...
export interface ConferenceClientDiagnosticsSnapshot {
  ...
  currentSession: ConferenceSessionDiagnosticsSnapshot | null;
}
...
export class ConferenceClient extends EventEmitter<ConferenceClientEvents> {
  ...
  private async _joinRealtimeKitWithRetry<TClient extends RetryableRealtimeKitClient>(
    ...
    return joinRealtimeKitWithRetry(rtkClientOrFactory, joinPolicySelection, {
      ...
      isRetryableJoinError: (error) => isRetryableRtkJoinError(error),
    ...
    return this.joinLock.withLock(async () => {
      ...
      try {
        const joined = await joinConferenceSession(sessionId, config, {
          ...
          joinRealtimeKitWithRetry: (rtkClientOrFactory, policySelection) => this._joinRealtimeKitWithRetry(rtkClientOrFactory, policySelection),
        });
  ...
  getDiagnosticsSnapshot(): ConferenceClientDiagnosticsSnapshot {
    return {
      ...
      currentSession: this.currentSession?.getDiagnosticsSnapshot() ?? null,
    };
  }
  ...
  disconnect(): void {
    const { nextSession, nextWsClient } = disconnectCurrentRoom(this.currentSession, this.currentWsClient);
