...
function DialogOverlay({
	...
	return (
		<DialogPrimitive.Backdrop
			...
			className={cn(
				"data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 bg-black/60 backdrop-blur-sm duration-300 fixed inset-0 isolate z-50",
				className,
			)}
			{...props}
		/>
...
function DialogClose({ ...props }: DialogPrimitive.Close.Props) {
	return (
		<DialogPrimitive.Close
			...
			className="absolute right-4 top-4 rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground transition-all outline-none focus-visible:ring-2 focus-visible:ring-primary z-50"
			{...props}
		>
			<X size={18} />
		</DialogPrimitive.Close>
