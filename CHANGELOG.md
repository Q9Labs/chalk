...
- **Whisper worker: expire replayed jobs + restore lean prod capacity** — worker now fails over-age queued/recovered jobs before re-downloading/transcribing them, preventing endless replay after API-side timeouts, and lean prod whisper defaults are restored to `c7i.xlarge` with `4` CPU threads to reduce long-recording stalls on `c7i.large`.
