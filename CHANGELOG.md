...
- **API: post-meeting transcription now supports Cloudflare Workers AI Whisper Large v3 Turbo** — Chalk API now ships a `cloudflare` transcription provider for post-meeting recordings, supports dedicated `POST_MEETING_CLOUDFLARE_*` credentials/model overrides with fallback to the existing Cloudflare account/token env vars, and honors the configured default transcription provider so managed Workers AI can replace the self-hosted Whisper path cleanly.
...
- **Infra/security: lean Cloudflare credentials are now split by responsibility** — Terraform now reads a dedicated control-plane token, RealtimeKit runtime wiring uses its own token, and post-meeting Workers AI gets a separate `POST_MEETING_CLOUDFLARE_API_TOKEN` in SSM so Chalk no longer reuses one broad Cloudflare token across infra, meetings, and transcription.

### Fixed

- **Mobile/dev: simulator dev-client sessions now avoid iOS clipboard crashes and Android split-component render failures** — iOS simulator dev builds now disable the Expo dev-client network inspector and skip eager clipboard reads that were tripping the paste/runtime path, the shared Metro launcher now restores Android `adb reverse` mappings for hot-reload sessions, and `sdk-react-native` split platform components now export the generic wrapper names Metro expects so Android/macOS platform resolution no longer returns `undefined` React elements after the platform UI split.
- **Infra/post-meeting: Cloudflare Worker deploys are now pinned to the paid account and CI writes Wrangler OAuth where the CLI actually reads it** — the transcription worker config now pins account `5281943bd26d5bdcf4c3915606cd6bfb`, and GitHub Actions writes Wrangler OAuth credentials to both runner config locations so queue creation, secret sync, and deploy no longer fail in CI while the live Worker uses the wrong or unauthenticated account context.
- **Infra/post-meeting: Worker CI now relies on refreshed Wrangler OAuth config for deploys instead of a drifting post-meeting token path** — the transcription Worker deploy job now uses the same working Wrangler OAuth auth path as manual production deploys, which avoids CI pointing at stale or under-scoped Cloudflare token credentials while still targeting the paid account.
- **Infra/post-meeting: Cloudflare Worker now normalizes MP4 recording MIME types before Workers AI transcription** — queue jobs now rewrite post-meeting recording inputs like `video/mp4` to audio-safe MIME types such as `audio/mp4` based on the fetched header and stored object path, which prevents valid MP4 recordings from falling into the Cloudflare Queue DLQ before transcript/summary/action-item generation can run.
- **Web/dashboard: recording playback, download, and share now use the real recording routes and summary empty states no longer spin forever** — the internal dashboard now fetches playback/download URLs from `/api/v1/recordings/:id/download`, creates share links from `/api/v1/recordings/:id/share`, and shows honest completed/failed summary states instead of treating missing summary/action-item data as an infinite in-progress state.
- **Mobile/release: tenant-agnostic production builds now ship without a bundled host API key** — Chalk mobile release tooling now treats the host key as optional, still forces production API/WS endpoints, strips duplicate CLI separators for local production wrapper commands, and aligns native version metadata at `1.0 (17)` so local signed Android artifacts and the iOS TestFlight upload can be produced directly on the Mac without reintroducing a secret-backed mobile tenant.
- **Mobile/CI: Android release workflows no longer fail on mobile-gate test resolution and local host-key verification now works on Node 25** — `apps/mobile` now gives Vitest explicit workspace aliases for `@q9labs/chalk-core` and `@q9labs/chalk-react-native`, keeps dev diagnostics state active under Vitest even when CI runs with `NODE_ENV=production`, and replaces the production host-key verifier’s top-level `await` with an explicit async entrypoint so release verification can run on newer local Node runtimes.
- **Mobile/iOS: simulator localhost dev runs no longer silently fall back to production** — the mobile runtime now preserves configured `localhost` API and WS URLs when the Expo dev bundle itself is served from `localhost`, preventing local iOS simulator sessions from flipping to production targets and failing host-key auth during create/join flows.
- **Mobile branding: legacy `Hasan Headquaters` app-name strings were removed from native/mobile surfaces** — the Expo app config, iOS display name and microphone permission copy, Android app label, and remaining headquarters UI strings now consistently use `Chalk`.

- **Web/SDK-React: local web dev no longer crashes on Chalk whiteboard imports or CSS font ordering** — `apps/web` now aliases `@q9labs/chalk-whiteboard` source entrypoints alongside the existing source-linked Chalk packages so Vite can resolve the optional whiteboard collab module during local development, and `sdk-react` no longer injects a late Google Fonts `@import` that PostCSS rejects after Tailwind-generated statements.
- **SDK-React: mobile prejoin name edits now submit correctly and hydrate safely on phones** — the mobile join sheet now treats the in-progress name as the source of truth when enabling/submitting `Ask to join`, forwards unsaved edits into the join action so users do not have to tap the save checkmark first, and `useMediaQuery` now hydrates from a server-safe default before syncing in a client layout effect so phone-sized SSR renders no longer swap the entire prejoin tree during hydration.
- **Repo/tests: Vitest migration now passes across web, mobile, and SDK packages** — Bun-style test imports/mocks were converted to Vitest-compatible patterns, `sdk-react` now uses a proper Vitest config plus safer browser-global setup, and web/mobile test helpers were aligned with the new auth/runtime APIs so the full gate runs cleanly after the package-manager migration.
...
- **SDK-Core: RTK `videoUpdate` no longer fans out background side-effects on duplicate track churn** — background suspend/reapply is now gated behind a real local video transition plus an actually selected effect, preventing repeated no-track `media.video.update` bursts from recursively detaching/reattaching middleware and causing visible local video re-render loops.
- **SDK-Core/SDK-React: background effects are temporarily disabled for stability** — Chalk now reports background transforms as unsupported at the SDK capability layer and the meeting-room controller no longer exposes the background-effects UI, removing the entire virtual-background path from the product until the RTK integration is safe to re-enable.

## [0.0.80] - 2026-03-27

### Added

- **Web/support: error dialog can now export a full client-side debug report** — ChalkWeb error dialogs now expose `Copy Full Debug` plus JSON download, backed by an in-memory runtime collector that captures browser-visible fetch requests/responses, websocket lifecycle/payload logs, console output, uncaught runtime errors, wide-events, SDK incidents/breadcrumbs, cookies/storage/env snapshots, and the active Chalk session state for support-heavy debugging.
...
- **Mobile/meeting room: screen share is enabled again in the app shell** — the native meeting room is no longer hard-wired with `features.screenShare=false`, and the diagnostics timeline now records why screen share is unavailable when a future config/runtime gate disables it.
...
- **Mobile diagnostics: “Copy Full Debug” now safely serializes and verifies clipboard writes** — the mobile debug sheet now sanitizes circular/non-JSON-safe values before building the exported payload, verifies the clipboard after writing, and records an explicit diagnostics failure if the copy action fails instead of silently doing nothing.
- **Mobile/RN UI: diagnostics callbacks no longer churn renders and meeting lists use stronger keys** — the mobile app now deduplicates repeated diagnostics snapshots before bouncing them back through React state, which prevents the `Maximum update depth exceeded` loop seen during repeated meeting diagnostics updates, and the RN meeting surfaces now use stronger composite keys for participant/reaction/gradient-stop lists so duplicate upstream ids do not trigger React key warnings.
- **SDK-Core/mobile+web: post-join auth now stays room-scoped and stale RTK screen-share flags self-heal** — once a participant joins, API refreshes and WebSocket reconnects now use the joined session token/refresh token instead of falling back to the app bootstrap token source, which restores WS-backed actions like hand raise/reactions/chat after reconnects; RTK participant mapping also now treats real remote screen-share tracks/producers as authoritative even when `screenShareEnabled` lags false, helping web/mobile receivers render shared screens from stale RN RTK snapshots.
...
- Mobile React Native session setup now keeps a stable diagnostics `wideEvents` config across harmless re-renders, so entering the lobby no longer thrashes `session.init` / `session.dispose` cycles and inflate join latency before the real room connection starts.
...
- Local Android APK install now auto-falls back to pushing the APK into the phone Downloads folder and opening it when MIUI blocks `adb install`.
- Local Android APK delivery now defaults to push-to-Downloads with a timestamp/random-suffixed filename so stale device files are easy to distinguish.
- Local Android APK push flow now triggers a media scan after `adb push`, so MIUI Files can see fresh APKs immediately instead of hiding them until a later refresh.
