...
- **Infra/post-meeting: Worker CI now reuses the repo’s primary Cloudflare API token instead of a divergent post-meeting token secret** — the transcription Worker deploy job now reads `CLOUDFLARE_API_TOKEN` directly so GitHub Actions uses the same paid-account credential that already powers the rest of Chalk’s Cloudflare automation, avoiding account drift between live manual deploys and CI.
