...
- **React SDK visual polish** - Refined design system for a more premium, polished feel
  ...
  - **Video tiles**: Organic speaking glow animation, smoother gradient overlays
  - **Tooltips**: Playful spring entrance animation
  - **Shadows**: Layered elevation system (`--chalk-shadow-1` through `--chalk-shadow-4`)
  - **Easing**: New timing functions (`--chalk-ease-out-expo`, `--chalk-ease-out-back`)
  - **Animations**: Added hand wave, recording pulse, scale spring, panel slide-in
  - **Icons**: Migrated UI package from `@hugeicons/react` to `lucide-react` for consistency
