...
- Fallback env vars: `GPLAY_SERVICE_ACCOUNT_JSON`, `GPLAY_PACKAGE`.
...
| Variable | Purpose |
|----------|---------|
| `GPLAY_SERVICE_ACCOUNT_JSON` | Path to service account JSON |
...
## Track / tester gotchas

- `gplay tracks list`, `gplay tracks get`, and `gplay testers get` require `--edit`
- pattern:

```bash
edit_id=$(GPLAY_SERVICE_ACCOUNT_JSON=/path/to/sa.json gplay edits create --package com.example.app | jq -r .id)
GPLAY_SERVICE_ACCOUNT_JSON=/path/to/sa.json gplay tracks list --package com.example.app --edit "$edit_id"
```

- if you need to repoint `internal` to an already-approved artifact, `gplay tracks update` can set `status: completed` with an existing `versionCodes` array, then `gplay edits commit`
