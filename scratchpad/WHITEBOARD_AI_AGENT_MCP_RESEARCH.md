...
`excalidraw/excalidraw-mcp` is an MCP App server meant to render an interactive Excalidraw diagram *inline in chat*, not to control an existing Excalidraw instance embedded in another product.
...
Why this matters: it is explicitly designed to let an agent iteratively manipulate a *live* canvas via element-level tools, including screenshot feedback. That aligns more closely with "AI draws on Chalk's whiteboard".
