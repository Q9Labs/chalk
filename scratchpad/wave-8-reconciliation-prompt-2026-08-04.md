...
- `0c6768c0` is the unique dashboard completion tail. It contains the Account
  and Tenant management workflows, Space and Episode management, API-key
  lifecycle, recent-auth enforcement, audit/trace behavior, and related web UI.
...
4. Extract only unique live Episode debugger behavior from the dirty source
   worktree. Reimplement it against final packages and names.
...
- `infrastructure/meeting-broker` or other files superseded by the Episode
  broker;
...
2. Dashboard source and its three unique migrations;
...
- Run the canonical `pnpm run gate` over the exact staged tree on the M4 host.
